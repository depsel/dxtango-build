...
import SwapHorizontalIcon from "mdi-react/SwapHorizontalIcon.js";
import ContentSave from "mdi-react/ContentSaveIcon.js";
...
import CalculatorIcon from "mdi-react/CalculatorIcon.js";
...
const FULL_NAME_MAX_LENGTH = FIRST_NAME_MAX_LENGTH + LAST_NAME_MAX_LENGTH + 1;
...
function isName(value, minLength, maxLength) {
  const re = /^[\p{L}\s]+$/iu;
  return typeof value === "string" && value.length >= minLength && value.length <= maxLength && re.test(value);
}
...
function forceToNumber(raw) {
  if (raw === void 0) return 0;
  let maybe = Number(raw);
  if (!Number.isFinite(maybe)) return 0;
  return maybe;
}
...
let _executionMode;
let _programVersion$1;
function _initEnvironmentConfig() {
  if (isNavigator()) {
    const executionModeString = window?.dxTangoExecutionMode;
    const programVersionString = window?.dxTangoProgramVersion;
    if (executionModeString) {
      _executionMode = stringToEnum(ExecutionMode, executionModeString);
    }
    _programVersion$1 = programVersionString ?? "0.0.0";
  }
  if (!isNode()) return;
  _programVersion$1 = global.dxTango.programVersion ?? "0.0.0";
  ...
  const currentDirectory = path$1.basename(process?.cwd?.() ?? "").toLowerCase();
...
function getExecutionMode() {
  if (_executionMode === void 0) {
    ...
    if (_executionMode === void 0) _throwEnvironmentFatalError();
  }
  return _executionMode;
}
function getProgramVersion() {
  if (_programVersion$1 === void 0) {
    _initEnvironmentConfig();
    if (_programVersion$1 === void 0) _throwEnvironmentFatalError();
  }
  return _programVersion$1;
}
...
async function mapEndpoint(req, params, endpoints) {
  ...
  switch (req.method) {
    case "GET":
      if (getEndpoint != null) return await getEndpoint.get(req, params);
    case "POST":
      if (postEndpoint != null) return await postEndpoint.post(req, params);
    case "PATCH":
      if (patchEndpoint != null) return await patchEndpoint.patch(req, params);
    case "PUT":
      if (putEndpoint != null) return await putEndpoint.put(req, params);
    case "DELETE":
      if (deleteEndpoint != null) return await deleteEndpoint.delete(req, params);
  }
...
const DEFAULT_COMPANY_NAME = APP_NAME;
const DEFAULT_WELCOME_MESSAGE = `Bienvenido a ${APP_NAME}`;
...
const DEFAULT_ADMIN_EMAIL = `admin@dxtango.local`;
const DEFAULT_SMTP_ENABLED = false;
const DEFAULT_SMTP_HOST = "localhost";
const DEFAULT_SMTP_PORT = 587;
const DEFAULT_SMTP_USE_TLS = false;
const DEFAULT_SMTP_FROM_EMAIL = `sender@dxtango.local`;
...
var ArticleGroupSeparator = /* @__PURE__ */ ((ArticleGroupSeparator2) => {
  ArticleGroupSeparator2[ArticleGroupSeparator2["none"] = 0] = "none";
  ArticleGroupSeparator2[ArticleGroupSeparator2["space"] = 1] = "space";
  ArticleGroupSeparator2[ArticleGroupSeparator2["dash"] = 2] = "dash";
  ArticleGroupSeparator2[ArticleGroupSeparator2["underscore"] = 3] = "underscore";
  ArticleGroupSeparator2[ArticleGroupSeparator2["lettersAndNumbers"] = 4] = "lettersAndNumbers";
  return ArticleGroupSeparator2;
})(ArticleGroupSeparator || {});
const DEFAULT_ORDER_SHOW_SHIPPING_DATE_TO_CUSTOMERS = true;
const DEFAULT_ORDER_CREATE_UPDATE_MESSAGE = "Sujeto a aprobación y disponibilidad de stock";
const DEFAULT_ARTICLE_EDIT_LIST_ENABLED = false;
const DEFAULT_ARTICLE_EDIT_LIST_GROUP_SEPARATOR = ArticleGroupSeparator.none;
...
class VONumberRange extends ValueObjectBase {
  ...
  validate(input) {
    ...
    try {
      ...
      if ((this.min == null || value >= this.min) && (this.max == null || value <= this.max)) return value;
    } catch (_2) {
    }
    this.throwError(input);
  }
...
class VOString extends ValueObject {
  validate(input) {
    if (!isStr(input)) this.throwError(input);
    return input;
  }
}
class VOTrimmedString extends ValueObject {
  ...
  validate(input) {
    ...
    if (this.maxLength == null || value.length <= this.maxLength) return value;
    this.throwError(input);
  }
}
class VONotEmptyString extends ValueObject {
  ...
  validate(input) {
    if (typeof input === "string") {
      const value = input.trim();
      if (value !== "" && (this.maxLength == null || value.length <= this.maxLength)) return value;
    }
    this.throwError(input);
  }
}
...
const SECOND_IN_MILLIS = 1e3;
const MINUTE_IN_MILLIS = MINUTE_IN_SECONDS * SECOND_IN_MILLIS;
const DAY_IN_MILLIS = DAY_IN_SECONDS * SECOND_IN_MILLIS;
function now() {
  return Date.now();
}
function addDays(date, days) {
  const ts = date.getTime();
  const millis = days * DAY_IN_MILLIS;
...
class VOEmail extends ValueObjectBase {
  constructor(address, rawDisplayName) {
    super();
    this.rawDisplayName = rawDisplayName;
    this.value = this.validate(address);
  }
  ...
  validate(rawAddress) {
    ...
    if (typeof address !== "string") this.throwInvalidAddressError(rawAddress);
    address = address.trim().toLowerCase();
    const isValidAddress = validator.isEmail(address, {
      domain_specific_validation: true
    });
    if (!isValidAddress) this.throwInvalidAddressError(rawAddress);
    const displayName = this.rawDisplayName?.trim();
    if (typeof displayName === "string" && !isName(displayName, 1, FULL_NAME_MAX_LENGTH)) {
      this.throwError(
        valueToString(this.rawDisplayName),
  ...
  toString() {
    const { address, displayName } = this.value;
    if (displayName != null) return `${displayName} <${address}>`;
    return address;
  }
  getAddress() {
...
class VOHexadecimal extends ValueObject {
  validate(input) {
    ...
    if (typeof input === "string") {
      const value = input.trim().toLowerCase();
      if (validator.isHexadecimal(value)) return value;
    }
    this.throwError(input);
  }
}
...
class VOUUIdV4 extends ValueObject {
  validate(input) {
    let value = input;
    if (typeof value != "string") this.throwError(input);
    try {
      if (validator.isUUID(value, 4)) return value;
    } catch (e) {
    }
    this.throwError(input);
  }
...
class VOUserRole extends ValueObject {
  validate(value) {
    try {
      ...
      if (isValidEnumValue(UserRole, value)) return value;
    } catch (_2) {
    }
...
class UserEntity extends RootEntity {
  constructor(unsafe) {
    ...
    this.email = email != null ? new VOEmailAddress(email) : void 0;
  ...
  toUnsafe() {
    const result = {
      ...
      vendedor_id: this.vendedorId?.valueOf(),
...
async function resolveUserStateException(user, tangoUserNotFoundErrorCode) {
  ...
  if (errorCode != null) {
    const miscSettings = await settingsProvider.getMiscSettings();
...
function validateSchema(schema, input, options) {
  ...
  for (const [field, validator2] of entries) {
    ...
    try {
      ...
      if (result2 instanceof ValueObjectBase) result2 = result2.valueOf();
...
const storedProgramConfigSchema = {
  ...
  company_name: (v) => tryVO(() => new VOString(v), new VOString(DEFAULT_COMPANY_NAME)),
  welcome_message: (v) => tryVO(() => new VOString(v), new VOString(DEFAULT_WELCOME_MESSAGE)),
  welcome_logo: (v) => tryVO(() => new VOString(v), new VOString(DEFAULT_WELCOME_LOGO))
};
...
const storedMiscSettingsSchema = {
  smtp_enabled: (v) => tryVO(() => new VOBoolean(v ?? DEFAULT_SMTP_ENABLED), new VOBoolean(DEFAULT_SMTP_ENABLED)),
  ...
  smtp_from_email: (v) => tryVO(() => new VOString(new VOEmailAddress(v).valueOf()), new VOString(DEFAULT_SMTP_FROM_EMAIL)),
  ...
  admin_password_hash: (v) => tryVO(() => new VONotEmptyString(v), new VONotEmptyString("BADBADBAD")),
  admin_email: (v) => tryVO(() => new VOString(new VOEmailAddress(v).valueOf()), new VOString(DEFAULT_ADMIN_EMAIL))
};
...
function numberToArticleGroupSeparator(value) {
  if (isANumber(value)) {
    switch (value) {
      case 0:
        return ArticleGroupSeparator.none;
      case 1:
        return ArticleGroupSeparator.space;
      case 2:
        return ArticleGroupSeparator.dash;
      case 3:
        return ArticleGroupSeparator.underscore;
      case 4:
        return ArticleGroupSeparator.lettersAndNumbers;
    }
  }
  throw new ValidationException(`Invalid article group separator number: ${valueToString(value, true)}`);
}
class VOArticleGroupSeparator extends ValueObject {
  validate(rawValue) {
    return numberToArticleGroupSeparator(rawValue);
  }
}
const storedOrderSettingsSchema = {
  article_edit_list_enabled: (v) => tryVO(() => new VOBoolean(v ?? DEFAULT_ARTICLE_EDIT_LIST_ENABLED), new VOBoolean(DEFAULT_ARTICLE_EDIT_LIST_ENABLED)),
  article_edit_list_group_separator: (v) => tryVO(() => new VOArticleGroupSeparator(v ?? DEFAULT_ARTICLE_EDIT_LIST_GROUP_SEPARATOR), new VOArticleGroupSeparator(DEFAULT_ARTICLE_EDIT_LIST_GROUP_SEPARATOR)),
  show_shipping_date_to_customers: (v) => tryVO(() => new VOBoolean(v ?? DEFAULT_ORDER_SHOW_SHIPPING_DATE_TO_CUSTOMERS), new VOBoolean(DEFAULT_ORDER_SHOW_SHIPPING_DATE_TO_CUSTOMERS)),
  order_create_update_message: (v) => tryVO(() => new VONotEmptyString(v), new VONotEmptyString(DEFAULT_ORDER_CREATE_UPDATE_MESSAGE))
};
class StoredOrderSettingsDTO extends DTO {
  validate(input) {
    return validateInput(storedOrderSettingsSchema, input);
  }
  toUnsafe() {
    return this.data;
  }
...
const programConfigFilename = path$1.join(process.cwd(), "dxtango.cfg");
const dbIniFilename = path$1.join(configPath, "db_settings.ini");
const miscIniFilename = path$1.join(configPath, "misc_settings.ini");
const orderIniFilename = path$1.join(configPath, "order_settings.ini");
async function _createDTOFromINI(filename, dtoConstructor, writeDefaultIfError) {
  let iniFileContent;
  try {
...
function _createDTOFromINISync(filename, dtoConstructor, writeDefaultIfError) {
  let iniFileContent;
  try {
...
async function _saveDTOToINI(filename, dto) {
  ...
  await mkdir(path$1.dirname(filename), { recursive: true });
...
class SettingsProvider {
  ...
  async init(ignoreCache) {
    ...
    await this.getOrderSettings(ignoreCache);
  }
  async getProgramConfig(ignoreCache) {
    if ((ignoreCache ?? false) || this._programConfig == null) this._programConfig = await this._loadProgramConfig();
  ...
  async isSystemDisabled() {
    ...
    if (disable_system_file.length == 0) return false;
    try {
  ...
  async setMiscSettings(settings2) {
    const oldSettings = this._miscSettings;
    const newSettings = await this._saveMiscSettings(settings2);
    this._miscSettings = newSettings;
    emitBusEvent(
      new MiscSettingsChangedEvent({
        oldSettings: oldSettings?.data,
        newSettings: newSettings.data
      })
    );
    return this._miscSettings.data;
  }
  async getOrderSettings(ignoreCache) {
    if ((ignoreCache ?? false) || this._orderSettings == null) this._orderSettings = await this._loadOrderSettings();
    return this._orderSettings.data;
  }
  async setOrderSettings(settings2) {
    const newSettings = await this._saveOrderSettings(settings2);
    this._orderSettings = newSettings;
    return this._orderSettings.data;
  }
  async _loadProgramConfig() {
    return await _createDTOFromINI(
      programConfigFilename,
      (data) => new StoredProgramConfigDTO(data)
    );
  }
  _loadProgramConfigSync() {
    return _createDTOFromINISync(
      programConfigFilename,
      (data) => new StoredProgramConfigDTO(data)
    );
  }
  async _loadDBSettings() {
    return await _createDTOFromINI(
      dbIniFilename,
      (data) => new StoredDBSettingsDTO(data)
    );
  }
  async _loadMiscSettings() {
    return await _createDTOFromINI(
      miscIniFilename,
      (data) => new StoredMiscSettingsDTO({
        admin_password_hash: getPasswordHash(DEFAULT_ADMIN_PASSWORD),
        ...data
      })
    );
  }
  async _loadOrderSettings() {
    return await _createDTOFromINI(
      orderIniFilename,
      (data) => new StoredOrderSettingsDTO(data)
    );
  }
  ...
  async _saveOrderSettings(config2) {
    return await _saveDTOToINI(orderIniFilename, config2);
  }
}
const settingsProvider = new SettingsProvider();
const projectDirectory = process.cwd();
let _packageJsonInfo = {};
let _packageJsonInfoLoaded = false;
let _programName = "Project Name";
let _programDescription = "Project Description";
let _programVersion = "0.0.1";
let _programBuild = "0";
const _workerId = 0;
function _loadPackageJsonInfo() {
  try {
    const pjson = parseJSON(readFileSync(join(projectDirectory, "package.json"), "utf8"));
    if (isStr(pjson.name)) _programName = pjson.name.toUpperCase();
    if (isStr(pjson.description)) _programDescription = pjson.description;
    if (isStr(pjson.version)) _programVersion = pjson.version;
    const semVer = new SemVer(_programVersion);
    if (Array.isArray(semVer.build) && semVer.build.length > 0) {
      _programBuild = semVer.build[0];
    } else {
      _programBuild = "1";
    }
    _packageJsonInfo = pjson;
    _packageJsonInfoLoaded = true;
  } catch (e) {
  }
}
function _loadChangeLog() {
  try {
    return readFileSync(join(projectDirectory, "CHANGELOG.md"), "utf8");
  } catch (e) {
    return "No disponible";
  }
}
function _loadPackageAndReturn(valueProvider) {
  if (!_packageJsonInfoLoaded) _loadPackageJsonInfo();
  return valueProvider();
}
const programNameFromNode = _loadPackageAndReturn(() => _programName);
const programDescriptionFromNode = _loadPackageAndReturn(() => _programDescription);
const programVersionFromNode = _loadPackageAndReturn(() => _programVersion);
const programBuildFromNode = _loadPackageAndReturn(() => _programBuild);
const workerIdFromNode = _loadPackageAndReturn(() => _workerId);
const packageJsonInfoFromNode = _loadPackageAndReturn(() => _packageJsonInfo);
const changeLogFromNode = _loadChangeLog();
global.dxTango = {
  programName: programNameFromNode,
  programDescription: programDescriptionFromNode,
  programVersion: programVersionFromNode,
  programBuildFromNode,
  workerId: workerIdFromNode,
  packageJsonInfo: packageJsonInfoFromNode
};
...
async function handleBrowserRequest(request, responseStatusCode, responseHeaders, remixContext) {
  await settingsProvider.getProgramConfig();
...
function getEnvScript(programConfig) {
  const executionMode = getExecutionModeString();
  return `<script>window.dxTangoExecutionMode="${executionMode}";window.dxTangoProgramVersion="${programVersionFromNode}";<\/script>`;
}
...
class AuthState {
  ...
  userOrThrow() {
    if (this instanceof AuthStateLoggedIn) return this.userInfo;
...
class DXTApiRequestState {
  ...
  errorInfoOrNull() {
    if (this instanceof DXTApiRequestStateError) return this.info;
    return null;
  }
  flat() {
    return this.map({
...
function urlJoin(base, ...paths) {
  ...
  if (paths?.length > 0) {
    for (let path2 of paths) {
      ...
      if (resultLastChar != "/") {
        if (pathFirstChar != "/") result += "/";
      } else {
        if (pathFirstChar == "/") path2 = path2.substring(1);
      }
      result += path2;
    }
  }
  ...
  async run(authState, setAuthState, app, onRetryCallback) {
    ...
    void response.map({
      ...
      error: async (errorState) => {
        ...
        if (this.errorCallback) this.errorCallback(apiErrorInfo);
      }
    });
  }
}
...
const NONEXISTENT_PRODUCT = "Inexistente";
...
function getSelectedValue(arrayOfObjects, selected, key) {
  if (arrayOfObjects.length === 1) return arrayOfObjects[0][key ?? "id"];
...
const URL_SETTINGS_ORDER_PATH = appPath("/settings/order");
...
const URL_VERSION_PATH = appPath("/version");
...
const variantBasicPrintable = definePartsStyle$5((props) => {
  return {
    table: {
      ...
      ".rows > tr:nth-of-type(even)": {
        background: `gray.50`,
        color: `black`
      },
      ".rows > tr:nth-of-type(odd)": {
        background: `white`,
        color: `black`
      },
      ".captions": {
...
async function loader$L({ request }) {
  ...
  const programConfig = await settingsProvider.getProgramConfig();
  return {
    cookies,
    programConfig
  };
...
const meta$1 = ({ data }) => [
  ...
  { title: data?.programConfig.company_name ?? DEFAULT_COMPANY_NAME }
];
function App() {
  ...
  let cookies = useLoaderData().cookies;
...
class ApiEndpoint {
  ...
  async run(req, options) {
    try {
      ...
      if (error) return await this.controller.onError(error, req);
  ...
  _validateAllRequestValues(req) {
    ...
    if (validation == null) return void 0;
    const result = {};
    const { body, query, params } = validation;
    if (body != null) result.body = this._validateRequestValue(0, "body", req.bodyJson, body);
    if (query != null) result.query = this._validateRequestValues(1, req.query, query);
    if (params != null) result.params = this._validateRequestValues(2, req.params, params);
    if (result.body === void 0 && result.query === void 0 && result.params === void 0) return void 0;
    return result;
  }
  ...
  _validateRequestValue(target, key, value, validatorFunction) {
    if (typeof validatorFunction !== "function") return void 0;
    try {
...
const dxtVendedorTablePostCommandsSQL = [
  `CREATE INDEX ix_dxt_vendedor_tango_id ON dbo.dxt_vendedor (tango_id);`
];
...
const isCacheDisabledGlobally = tryVO(() => new VOBoolean(settingsProvider.getProgramConfigSync()?.disable_cache), new VOBoolean(false)).valueOf();
const CACHE_AUTOCLEAN_INTERNAL_MILLIS = DAY_IN_MILLIS;
class MemoryCache {
  constructor(_cacheOwnerStatus) {
    ...
    if (!isCacheDisabledGlobally) {
      eventBus.on(DBSettingsChangedEvent.name, this._onDBSettingsChanged.bind(this));
      eventBus.on(DBTablesChangedEvent.name, this._onTablesChanged.bind(this));
    }
  }
  _consoleLog(...args) {
    if (isDevelopment()) {
  ...
  async _autoClear() {
    const now2 = Date.now();
    if (now2 - this._lastClear < CACHE_AUTOCLEAN_INTERNAL_MILLIS) return;
    this._consoleLog(`Cache._autoClear()`);
    await this.clear();
  }
  async isReady() {
    if (isCacheDisabledGlobally || !await this._cacheOwnerStatus.isReady()) {
      return false;
    }
    await this._autoClear();
    return true;
  }
  async getByIdBuffer() {
...
const DEPOSITO_NAME_COLUMNS = ["NOMBRE_SUC"];
...
const PEDIDO_CLIENTE_TAXES_COLUMNS = [
  "II_L",
  "IVA_D",
  "IVA_L",
  "SOBRE_II",
...
class DictionaryRepository extends TDBProvider {
  ...
  async getDictionary() {
    ...
    const settings2 = await settingsProvider.getDBSettings();
  ...
  async getNumeroPedidoLongitud() {
    await this.getDictionary();
    if (this._numeroPedidoLongitud == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "DictionaryProvider.getNumeroPedidoLongitud");
    return this._numeroPedidoLongitud;
  }
  async getNumeroPedidoRange() {
    await this.getDictionary();
    if (this._numeroPedidoRange == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "DictionaryProvider.getNumeroPedidoRange");
...
class VOOrderRows extends ValueObject {
  validate(input) {
    if (!Array.isArray(input)) this.throwError(input, "Input is not an array");
    return input.map((e) => {
      if (!isObj(e)) this.throwError(input);
      const { id, quantity } = e;
      if (!isAnInteger(id)) this.throwError(input, "Id field missing or invalid");
      if (!isANumber(quantity)) this.throwError(input, "Quantity field missing or invalid");
      return { id, quantity };
    });
  }
}
...
class VOIntegers extends ValueObject {
  validate(input) {
    ...
    return values.map((v) => {
      if (typeof v === "string") v = Number.parseInt(v);
...
const LEYENDA_MAX_LENGTH = 60;
const DESCRIPCION_MAX_LENGTH = 120;
const MENSAJE_INTERNO_FIELD_LENGTH = 240;
const baseInputSchema = {
  leyenda1: (v) => new VOTrimmedString(v, LEYENDA_MAX_LENGTH),
  leyenda2: (v) => new VOTrimmedString(v, LEYENDA_MAX_LENGTH),
  leyenda3: (v) => new VOTrimmedString(v, LEYENDA_MAX_LENGTH),
  leyenda4: (v) => new VOTrimmedString(v, LEYENDA_MAX_LENGTH),
  leyenda5: (v) => new VOTrimmedString(v, LEYENDA_MAX_LENGTH),
  mensaje_interno: (v) => new VOTrimmedString(v, MENSAJE_INTERNO_FIELD_LENGTH),
...
function _extraValidation(values) {
  ...
  if (fecha_entrega < fecha_alta) throw new DXTException(DXTErrorCode.DELIVERY_DATE_IS_EARLIER);
}
...
const dxtPedidoDraftModelColumns = [
  ...pedidoModelColumns,
  "MENSAJE_INTERNO"
];
const dxtPedidoDraftTableCreationFieldsSQL = {
  ...
  LEYENDA_1: `varchar(${LEYENDA_MAX_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`,
  LEYENDA_2: `varchar(${LEYENDA_MAX_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`,
  LEYENDA_3: `varchar(${LEYENDA_MAX_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`,
  LEYENDA_4: `varchar(${LEYENDA_MAX_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`,
  LEYENDA_5: `varchar(${LEYENDA_MAX_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`,
  ...
  MENSAJE_INTERNO: `varchar(${MENSAJE_INTERNO_FIELD_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`
};
...
const TERMINAL_MODIFICACION_PRECIO_ESPECIAL = "DXTE";
...
const renglonPedidoArticuloModelColumns = [
  "DESCRIPCIO",
  "DESC_ADIC",
  "COD_IVA",
  "COD_S_IVA",
...
const DXT_PEDIDO_EXTRA_TABLE = "dxt_pedido_extra";
const dxtPedidoExtraModelColumns = [
  "MENSAJE_INTERNO"
];
const dxtPedidoExtraTableCreationFieldsSQL = {
  [PEDIDO_ID_FIELD]: "int NOT NULL",
  MENSAJE_INTERNO: `varchar(${MENSAJE_INTERNO_FIELD_LENGTH}) COLLATE Latin1_General_BIN DEFAULT '' NULL`
  // UPDATED_AT: `datetime DEFAULT sysdatetime() NOT NULL`,
};
const dxtPedidoExtraTableCreationIndexesSQL = [
  `CONSTRAINT PK_DXT_PEDIDO_EXTRA_id PRIMARY KEY (${PEDIDO_ID_FIELD})`
];
const dxtPedidoExtraTablePostCommandsSQL = [
  // `CREATE INDEX IX_DXT_PEDIDO_EXTRA_updated_at ON DXT_PEDIDO_EXTRA (UPDATED_AT ASC);`,
];
class CompanyInitProvider extends TDBProvider {
  ...
  async _queryChangesAndContinue(knexInstance, dbName) {
    if (isCacheDisabledGlobally) return knexInstance;
  ...
  async _initializeTables(knex2) {
    await this._initializeTable(
      knex2,
      DXT_PEDIDO_EXTRA_TABLE,
      dxtPedidoExtraTableCreationFieldsSQL,
      dxtPedidoExtraTableCreationIndexesSQL,
      dxtPedidoExtraTablePostCommandsSQL
    );
    await this._initializeTable(
      knex2,
      DXT_CLIENTE_TABLE,
    ...
    await this._initializeTable(
      knex2,
      DXT_VENDEDOR_TABLE,
      dxtVendedorTableCreationFieldsSQL,
      dxtVendedorTableCreationIndexesSQL,
      dxtVendedorTablePostCommandsSQL
    );
...
const _CompanyProvider = class _CompanyProvider extends CompanyInitProvider {
  constructor(config2) {
    ...
    if (dependencies != null) _collectDependencies(dependencies, _collectedProviders);
    this._dependencies = [
      mainTable,
  ...
  async count() {
    ...
    if (!validCount) throw new DXTException(DXTErrorCode.FATAL_DB_QUERY_FAILED, `CompanyProvider.count() on ${mainTable}`);
    return data[0].count;
  }
  async _getById(id, options) {
    const k = options?.trx ?? await this.getCompany();
    const useCache = options?.useCache ?? true;
    if (useCache) {
      const cachedData = await this.cache.getById(id);
      if (cachedData != null) return cachedData;
    }
    const { mainTable, mainIdField, parallelTable } = this.config;
    const columns = this._prepareColumns(this.config);
    let query = k(mainTable).first(...columns ?? "*");
    if (parallelTable != null) {
      const { name } = parallelTable;
      query = query.leftOuterJoin(name, `${mainTable}.${mainIdField}`, `${name}.${mainIdField}`);
    }
    const data = id != null ? await query.where(`${mainTable}.${mainIdField}`, id) : await query;
  ...
  _prepareColumns(config2) {
    const { mainTable, columns, parallelTable } = config2;
    if (parallelTable == null) {
      return columns ?? null;
    }
    const parallelColumns = parallelTable.columns.map((c) => `${parallelTable.name}.${c}`);
    const allColumns = [
      ...parallelColumns.length > 0 && columns != null ? columns.map((c) => `${mainTable}.${c}`) : [],
      ...parallelColumns
    ];
    return allColumns.length > 0 ? allColumns : null;
  }
  async getMany(columnName, columnValue, options) {
    ...
    if (where != null) {
      query = query.where(`${mainTable}.${where.field}`, where.value);
    }
  ...
  async getOne(columnName, columnValue, options) {
    ...
    if (result == null) throw new DXTException(DXTErrorCode.NOT_FOUND);
    return result;
  }
  ...
  getAllByUserCacheKey(role, tangoUserId, options) {
    ...
    return `${this.config.mainTable}_BTU_${this.getOptionsCacheKey(options)}_${role}_${tangoUserId ?? "A"}`;
  }
  getAllListByUserCacheKey(role, tangoUserId, options) {
    const useCache = options?.useCache ?? true;
    if (!useCache) return null;
    return `${this.config.mainTable}_LBTU_${this.getOptionsCacheKey(options)}_${role}_${tangoUserId ?? "A"}`;
  }
  _getByColumnCacheKey(columnName, columnValue, extraKey, options) {
    ...
    const { mainTable } = this.config;
    return `${mainTable}_BC_${this.getOptionsCacheKey(options)}_${extraKey}_${finalColumnName}_${finalColumnValue}`;
  }
  getAllCacheKey(options) {
    const useCache = options?.useCache ?? true;
    if (!useCache) return null;
    const { mainTable } = this.config;
    return `${mainTable}_ALL_${this.getOptionsCacheKey(options)}`;
  }
  getOptionsCacheKey(options) {
    if (options == null) return "NO";
    const {
      offset,
      limit,
      where,
...
function _collectDependencies(providers, collectedProviders) {
  providers.forEach((p) => {
    ...
    if (providerAlreadyExists) return;
    collectedProviders.push(p);
    if (isStr(p)) return;
    const { dependencies } = p.config;
    if (dependencies != null) _collectDependencies(dependencies, collectedProviders);
  });
}
...
const tangoDepositoModelMapper = (m) => {
  const { [DEPOSITO_ID_FIELD]: id, [DEPOSITO_CODE_FIELD]: code, NOMBRE_SUC, INHABILITA: inhabilitado } = m;
  return { id, code, name: NOMBRE_SUC ?? code, habilitado: inhabilitado !== true };
};
...
const tangoAsientoModelMapper = (m) => {
  ...
  if (!isAnInteger(id)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, `tangoAsientoModelMapper -> ${ASIENTO_ID_FIELD}`);
  if (!isAnInteger(codigo_cuenta)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, `tangoAsientoModelMapper -> codigo_cuenta`);
  if (!isNotEmptyStr(tipo)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, `tangoAsientoModelMapper -> ${ASIENTO_CODE_FIELD}`);
...
const tangoPerfilModelMapper = (m) => {
  ...
  if (!isAnInteger(D_TAL_FACT)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> D_TAL_FACT");
  if (!isAnInteger(D_TAL_PED)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> D_TAL_PED");
  if (!isAnInteger(D_TAL_REMI)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> D_TAL_REMI");
  if (!isAnInteger(codigo_condicion_venta)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> codigoCondicionVenta");
  if (!isAnInteger(D_LISTA_PR)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> D_LISTA_PR");
  if (!isNotEmptyStr(D_TIPO_ASI)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> D_TIPO_ASI");
  if (!isAnInteger(ID_ASIENTO_MODELO_GV)) throw new DXTException(DXTErrorCode.TANGO_PERFIL_DB_DATA_ERROR, "tangoPerfilModelMapper -> ID_ASIENTO_MODELO_GV");
  return {
    code,
    id,
    name: DESCRIPCIO,
    ...
    fecha_alta_visible: true,
    fecha_entrega_visible: true,
    talonario_visible: _isVisible(TAL_FACTUR),
    asiento_visible: _isVisible(TIPO_ASIEN),
    transporte_visible: _isVisible(TRANSPORTE) || !isNotEmptyStr(D_TRANSPOR?.trim()),
    condicion_visible: _isVisible(COND_VENTA),
    deposito_visible: _isVisible(DEPOSITO),
    direccion_visible: _isVisible(EDITA_DIRECCION_ENTREGA),
    fecha_alta_editable: false,
    fecha_entrega_editable: false,
    talonario_editable: _isEditable(TAL_FACTUR),
    asiento_editable: _isEditable(TIPO_ASIEN),
    transporte_editable: _isEditable(TRANSPORTE) || !isNotEmptyStr(D_TRANSPOR?.trim()),
...
function _isEditable(field) {
  if (!isStr(field)) return false;
  return field.trim().toUpperCase() === "E";
}
function _isVisible(field) {
  if (!isStr(field)) return false;
  const c = field.trim().toUpperCase();
  return c === "M" || c === "E";
}
...
const tangoExtendedClienteModelMapper = (m) => {
  ...
  if (!isAnInteger(id_vendedor)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper -> ${VENDEDOR_ID_FIELD}`);
  if (!isNotEmptyStr(codigo_vendedor)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper ${VENDEDOR_CODE_FIELD}`);
  if (!isAnInteger(id_lista)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper ${LISTA_ID_FIELD}`);
  if (!isAnInteger(codigo_lista)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper NRO_LISTA`);
  if (!isAnInteger(id_condicion)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper ${CONDICION_ID_FIELD}`);
  if (!isAnInteger(codigo_condicion)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `tangoExtendedClienteModelMapper ${CONDICION_CODE_FIELD}`);
  return {
    ...
    id_lista,
    codigo_lista,
    discrimina_iva: taxFieldIsYes(IVA_D),
    exento_iva: true,
    // taxFieldIsExempt(IVA_L),
    exento_ii: true,
    // taxFieldIsExempt(II_L),
    aplica_sobre_iva: taxFieldIsYes(SOBRE_IVA),
    aplica_sobre_ii: taxFieldIsYes(SOBRE_II),
...
class ClienteRepository extends TangoProvider {
  ...
  async getExtendedByIdOrNull(customerId) {
  ...
  async getExtendedById(customerId) {
    const result = await this.getExtendedByIdOrNull(customerId);
    if (result == null) throw new DXTException(DXTErrorCode.DXT_CUSTOMER_NOT_FOUND);
    return result;
  }
  toResult(m) {
...
class DXTUserRepository extends CompanyProvider {
  ...
  async _getUserWithRelationsByWhere(knexInstance, field, value, tangoUserRequired, cacheKey) {
    ...
    if (tangoUserRequired && !tangoUserExists) throw new DXTException(tangoUserNotFoundErrorCode);
    const unsafe = this.toResultWithRelations(modelAndRelations);
    if (tangoUserExists && cacheKey != null) this.cache.setMetadata(cacheKey, unsafe);
    return unsafe;
  }
  ...
  async _assertValidInput(input, current_dxt_user_id) {
    ...
    if (profile == null || customer == null) throw new DXTException(DXTErrorCode.INVALID_PARAMETER);
  }
  async _throwIfUserNameExists(username, current_dxt_user_id) {
    if (current_dxt_user_id != null) {
      ...
      if (currentTableUser != null && currentTableUser.id != current_dxt_user_id) throw new DXTException(DXTErrorCode.USERNAME_ALREADY_EXISTS);
    }
    const { twinRepository } = this.config;
    const twinTableUser = await twinRepository().getOneOrNull("username", username);
    if (twinTableUser != null) throw new DXTException(DXTErrorCode.USERNAME_ALREADY_EXISTS);
  }
  ...
  async getEmailsByTangoId(tangoCustomerId, options) {
    const result = await this.getMany("tango_id", tangoCustomerId, options);
    return result.map((r) => ({
      dxtUserId: r.id,
...
class DXTVendedorRepository extends DXTUserRepository {
  ...
  toResult(m) {
    ...
    if (dxt_vendedor_id == null) throw Error("DXTVendedorRepository.toResult() -> dxt_vendedor_id == null");
    return {
      ...m,
      password_hash,
      id: dxt_vendedor_id,
...
class DXTClienteRepository extends DXTUserRepository {
  ...
  toResult(m) {
    ...
    if (dxt_cliente_id == null) throw Error("DXTClienteRepository.toResult() -> dxt_cliente_id == null");
    return {
      ...remaining,
      id: dxt_cliente_id,
      password_hash,
  ...
  async getSiblings(dxt_cliente_id, tango_id) {
    ...
    if (dxt_cliente_id == null) return result;
...
class RootController extends ApiController {
  ...
  async onRequest(req) {
    ...
    this.systemDisabled = await settingsProvider.isSystemDisabled();
    if (this.systemDisabled) {
      const message = await settingsProvider.getSystemDisabledMessage();
...
const _tokensPath = path__default.join(cachePath, "tokens");
class AuthTokenRepository {
  ...
  async _loadTokensFromFiles() {
    ...
    const settings2 = await settingsProvider.getMiscSettings();
    const expirationInMillis = settings2.auth_expiration_days * (isDevelopment() ? MINUTE_IN_MILLIS : DAY_IN_MILLIS);
    const currentTime = now();
...
async function connectWorker(authTokenString) {
  ...
  if (role === UserRole.customer) return await _testUserConnect(dxtClienteRepository, token, DXTErrorCode.TANGO_CUSTOMER_NO_LONGER_EXISTS);
  if (role === UserRole.seller) return await _testUserConnect(dxtVendedorRepository, token, DXTErrorCode.TANGO_SELLER_NO_LONGER_EXISTS);
...
async function _testUserLogin(repository, username, passwordHash, tangoUserNotFoundErrorCode) {
  ...
  if (passwordHash !== userPasswordHash) return void 0;
...
async function changePasswordWorker(user, oldPassword, newPassword) {
  ...
  if (role == UserRole.admin) {
    const settings2 = await settingsProvider.getMiscSettings();
    await settingsProvider.setMiscSettings(
...
class AuthenticatedRootController extends RootController {
  async onRequest(req) {
    ...
    if (typeof token !== "string") token = req.query["auth"];
...
const baseUserInputSchema = {
  ...
  username: (v) => {
    ...
    if (s.toLowerCase() == ADMIN_USERNAME) throw new DXTException(DXTErrorCode.FORBIDDEN_ADMIN_USERNAME);
    return s;
  },
...
function strToDXTArticuloListRecord(s) {
  ...
  if (codigo_articulo == ARTICLE_GROUP_NO_NAME) throw new DXTException(DXTErrorCode.RESERVED_ARTICLE_GROUP_NAME, `"${ARTICLE_GROUP_NO_NAME}"`);
...
function strToDXTArticuloListParams(paramsStr) {
  if (paramsStr == null) return void 0;
  const result = {};
...
function applyTangoDiscount(price, discount) {
  if (price == null) return null;
  if (!isANumber(price)) price = 0;
  if (!isANumber(discount)) discount = 0;
  discount = limitNumber(discount, 0, 100);
  return price - price * discount / 100;
}
function invertedTangoDiscount(price, priceListDiscount, orderDiscount) {
  if (!isANumber(price)) price = 0;
  if (priceListDiscount == null) return price;
  if (!isANumber(priceListDiscount)) priceListDiscount = 0;
  if (!isANumber(orderDiscount)) orderDiscount = 0;
  priceListDiscount = limitNumber(priceListDiscount, 0, 100);
  orderDiscount = limitNumber(orderDiscount, 0, 100);
  price = price - price * priceListDiscount / 100;
  if (orderDiscount >= 100) return 0;
  return price * 100 / (100 - orderDiscount);
}
...
function tangoFullArticleModelMapper(m, listId1, discount1, listId2, discount2) {
  ...
  const priceListDiscount = idLista === listId1 ? discount1 : idLista === listId2 ? discount2 : null;
  const articleDiscount = DESCUENTO ?? 0;
  const precio = applyTangoDiscount(PRECIO, articleDiscount);
  return {
    ...tangoArticuloModelMapper(m),
    id_lista: idLista,
    precio,
    bonificacion_lista_de_precios: priceListDiscount ?? void 0,
    codigo_iva: COD_IVA,
...
function tangoFullArticleOrGroupNameModelMapper(m, listId1, discount1, listId2, discount2) {
  ...
  if (isGroupLabel) return DXTO_CODIGO_ARTICULO.trim();
...
const dxtVendedorGetCustomersEndpoint = createApiEndpoint(
  ...
  async (req) => {
    ...
    if (!user.role.isSeller()) throw new DXTException(DXTErrorCode.SELLER_ROLE_REQUIRED);
...
const CONFIGURACION_DEL_SISTEMA = "Configuración del sistema";
...
const SETTINGS_UPDATED = "Configuraciones actualizadas";
const VARIOUS_SETTINGS = "Configuraciones Varias";
const ORDER_SETTINGS = "Pedidos";
...
const ORDER_CREATED_TITLE = "Pedido {{numero_pedido}} creado";
const ORDER_UPDATED_TITLE = "Pedido {{numero_pedido}} actualizado";
const DRAFT_CREATED_TITLE = "Borrador creado";
const DRAFT_CREATED_DESCRIPTION = "Descripción: {{descripcion}}";
const DRAFT_UPDATED_TITLE = "Borrador actualizado";
const DRAFT_UPDATED_DESCRIPTION = "Descripción: {{descripcion}}";
const settings = {
  customers: {
    api: {
...
const buildSelectOptions = (props) => {
  ...
  return result.sort(({ label: aLabel }, { label: bLabel }) => aLabel.localeCompare(bLabel));
};
...
const DXTUserEditReady = (props) => {
  ...
  const onSubmit = async (dataUnsafe) => {
    ...
    result.map({
      ...
      error: (e) => {
        toast({
          title: e.errorInfoOrNull()?.error_description ?? AN_ERROR_OCCURRED,
          status: "error"
        });
...
function yupValidation(validator2) {
  try {
    validator2();
    return true;
  } catch (_2) {
...
const baseUserValidation = {
  ...
  username: yup.string().required("Ingrese un nombre de usuario").test("username", "Ingrese un nombre de usuario válido", (v) => {
    if (!yupVOValidation(VOUserName, v)) return false;
...
function Edit() {
  const navigate = useNavigate();
  const { id } = useParams();
  const typeSettings = settings.sellers;
  const idValidated = tryVOValue(() => new VOInteger(id), null);
  if (idValidated == null) {
    return /* @__PURE__ */ jsx(
      CommonErrors,
...
function numberToEstadoPedido(value) {
  if (!Number.isInteger(value)) return EstadoPedido.INVALIDO;
  switch (value) {
    case EstadoPedido.INGRESADO:
...
function realOrderStatus(header, rows) {
  ...
  if (!canBeInProgress) return estado;
  for (const row of rows) {
    const { a_descontar, a_facturar, pendiente_descontar, pendiente_facturar } = row;
    if (a_descontar != pendiente_descontar || a_facturar != pendiente_facturar) return EstadoPedido.EN_PROGRESO;
  }
  return estado;
}
...
function isUserAllowedToModifyOrder(user, orderStatus) {
  ...
  if (orderStatus != EstadoPedido.INGRESADO) return false;
  return true;
}
function isUserAllowedToCancelOrDeleteOrder(user, orderStatus) {
  ...
  if (!puedeAnular) return false;
  if (orderStatus == EstadoPedido.INGRESADO) return true;
  const aprobarAlCrear = user instanceof UserEntity ? user.aprobarPedidoAlCrear.valueOf() : user.aprobar_pedido_al_crear;
  if (orderStatus == EstadoPedido.APROBADO && (isAdmin || aprobarAlCrear)) return true;
  if ((orderStatus == EstadoPedido.EN_PROGRESO || orderStatus == EstadoPedido.ANULADO) && isAdmin) return true;
  return false;
}
function orderCaptionsToArray(c1, c2, c3, c4, c5) {
  const result = [];
  if (c1 != null && c1.length > 0) result.push({ k: 1, v: c1 });
  if (c2 != null && c2.length > 0) result.push({ k: 2, v: c2 });
  if (c3 != null && c3.length > 0) result.push({ k: 3, v: c3 });
  if (c4 != null && c4.length > 0) result.push({ k: 4, v: c4 });
  if (c5 != null && c5.length > 0) result.push({ k: 5, v: c5 });
  return result;
}
...
function composeDateTime(date, time) {
  const [h, m, s] = _getHMS(time);
  const result = getDateOnly(date);
  result.setHours(h, m, s, 0);
  return result;
}
function _getHMS(hms) {
  if (!isStr(hms)) return [0, 0, 0];
  hms = hms.trim();
  if (hms.length != 6) return [0, 0, 0];
...
const pedidoModelMapper = (m) => {
  const {
    ...
    [TRANSPORTE_ID_FIELD]: id_transporte,
    [TRANSPORTE_CODE_FIELD]: codigo_transporte,
    [DIRECCION_ID_FIELD]: id_direccion_entrega,
    FECHA_PEDI,
    HORA,
    FECHA_INGRESO,
    HORA_INGRESO,
  ...
  if (!(FECHA_PEDI instanceof Date)) throw new DXTException(DXTErrorCode.TANGO_PEDIDO_DB_DATA_ERROR, "pedidoModelMapper -> FECHA_PEDI");
  if (!(FECHA_ENTR === null || FECHA_ENTR instanceof Date)) throw new DXTException(DXTErrorCode.TANGO_PEDIDO_DB_DATA_ERROR, "pedidoModelMapper -> FECHA_ENTR");
  if (!(FECHA_INGRESO === null || FECHA_INGRESO instanceof Date)) throw new DXTException(DXTErrorCode.TANGO_PEDIDO_DB_DATA_ERROR, "pedidoModelMapper -> FECHA_INGRESO");
  if (!(FECHA_ULTIMA_MODIFICACION === null || FECHA_ULTIMA_MODIFICACION instanceof Date)) throw new DXTException(DXTErrorCode.TANGO_PEDIDO_DB_DATA_ERROR, "pedidoModelMapper -> FECHA_ULTIMA_MODIFICACION");
  const fecha_hora_alta = composeDateTime(FECHA_PEDI, HORA);
  const fecha_hora_ingreso = composeDateTime(FECHA_INGRESO, HORA_INGRESO);
  const fecha_hora_modificacion = composeDateTime(FECHA_ULTIMA_MODIFICACION, HORA_ULTIMA_MODIFICACION);
  return {
    id: id_pedido,
    numero_pedido,
    ...
    id_talonario: id_talonario ?? 0,
    codigo_talonario: codigo_talonario ?? 0,
    id_condicion: id_cliente ?? 0,
    codigo_condicion: codigo_condicion ?? 0,
    id_lista: id_lista ?? 0,
    codigo_lista: codigo_lista ?? 0,
...
const pedidoAndTaxesModelMapper = (m) => {
  const r = pedidoModelMapper(m);
  const cliente_aplica_sobre_iva = taxFieldIsYes(m.SOBRE_IVA);
  const cliente_aplica_sobre_ii = taxFieldIsYes(m.SOBRE_II);
  const cliente_exento_ii = true;
  const cliente_exento_iva = true;
  return {
    ...r,
    cliente_aplica_sobre_iva,
    cliente_aplica_sobre_ii,
    cliente_exento_iva,
    cliente_exento_ii
  };
};
const pedidoAndRelationsModelMapper = (m) => {
  const r = pedidoAndTaxesModelMapper(m);
  const nombre_cliente = resolveScreenName(CLIENTE_NAME_COLUMNS, m);
  const nombre_vendedor = resolveScreenName(VENDEDOR_NAME_COLUMNS, m);
  const nombre_transporte = r.codigo_transporte != null ? resolveScreenName(TRANSPORTE_NAME_COLUMNS, m) : void 0;
  const nombre_deposito = m.NOMBRE_SUC?.trim();
  const direccion_cliente = m.DIRECCION;
  let localidad_cliente = void 0;
  if (m.LOCALIDAD != null) localidad_cliente = m.LOCALIDAD.trim();
  if (localidad_cliente != null) {
    localidad_cliente = localidad_cliente.trim();
    if (localidad_cliente == "") localidad_cliente = void 0;
  }
  let telefono_cliente = void 0;
  if (m.TELEFONO1 != null) telefono_cliente = m.TELEFONO1.trim();
  if (m.TELEFONO2 != null) {
    if (telefono_cliente != null && telefono_cliente.length) telefono_cliente += " ";
    else telefono_cliente = "";
    telefono_cliente += m.TELEFONO2;
  }
  if (telefono_cliente != null) {
    telefono_cliente = telefono_cliente.trim();
    if (telefono_cliente == "") telefono_cliente = void 0;
  }
  return {
    ...r,
    nombre_cliente,
...
const tangoParametrosModelMapper = (m) => {
  ...
  if (proximoId == null || !Number.isFinite(proximoId)) throw new DXTException(DXTErrorCode.TANGO_DB_INVALID_DATA, "tangoParametrosModelMapper -> PROX_NDOC");
  return {
...
class PedidoBaseRepository extends CompanyProvider {
  constructor(config2) {
    const { mainTable, rowsRepository, dependencies, parallelTable, columns } = config2;
    super({
      mainTable,
      mainIdField: PEDIDO_ID_FIELD,
      columns: columns ?? pedidoModelColumns,
      dependencies: [
        ...dependencies ?? [],
        rowsRepository
      ],
  ...
  //   user: UserEntity,
  //   orderId: number,
  //   options?: GetRecordBasicOptions
  // ): Promise<PedidoAndRelationsResult> {
  //   const cacheKey = this._getByIdWithRelationsCacheKey(orderId, options);
  //   if (cacheKey!=null) {
  ...
  //   const { mainTable, mainIdField, columns } = this.config;
  //   const k = await this.getCompany();
  //   let query = k(mainTable).where( `${mainTable}.${mainIdField}`, orderId );
  ...
  //   const direccionColumns = DIRECCION_BASIC_COLUMNS.map(c => `${DIRECCION_TABLE}.${c}`);
  //   const depositoNombreColumns = DEPOSITO_NAME_COLUMNS.map(c => `${DEPOSITO_TABLE}.${c}`);
  //   const data = await query
  //     .andWhereRaw(this._avoidInvalidsFilterWhereRaw())
  //     .first(
  //       ...mainColumns,
  //       ...pedidoClienteColumns,
  //       ...vendedorNombreColumns,
  //       ...transporteNombreColumns,
  //       ...direccionColumns,
  //       ...depositoNombreColumns
  //     )
  ...
  //     .leftOuterJoin(DIRECCION_TABLE, `${mainTable}.${DIRECCION_ID_FIELD}`, `${DIRECCION_TABLE}.${DIRECCION_ID_FIELD}`)
  //     .leftOuterJoin(DEPOSITO_TABLE, `${mainTable}.${DEPOSITO_CODE_FIELD}`, `${DEPOSITO_TABLE}.${DEPOSITO_CODE_FIELD}`)
  //   ;
  //   if (data == null) throw new DXTException(DXTErrorCode.NOT_FOUND);
  //   const result = await this.finalTransformer(user, this.toResultWithRelations(data) )
  //   if (cacheKey!=null) await this.cache.setMetadata(cacheKey, result);
  //   return result;
  // }
  // private _getByIdWithRelationsCacheKey(orderId: number, options: GetRecordBasicOptions|undefined): string|null {
  //   const useCache = options?.useCache ?? true;
  //   if (!useCache) return null;
  //   return `${this.config.mainTable}_BIDR_${orderId}`;
  // }
  async getAllByUser(user, options) {
    const userRole = user.role.valueOf();
    const tangoUserId = user.tangoId?.valueOf();
    const cacheKey = this.getAllByUserCacheKey(userRole, user.id.valueOf(), options);
    if (cacheKey != null) {
      const cachedData = await this.cache.getMetadata(cacheKey);
      if (cachedData) {
        const settings22 = await settingsProvider.getOrderSettings();
        return this._removePrivateFields(userRole, cachedData, settings22);
      }
    }
    const { mainTable, columns, parallelTable } = this.config;
    const k = await this.getCompany();
    const where = this._getUserRoleWhere(userRole, tangoUserId);
    let query = k(mainTable);
    ...
    const direccionColumns = DIRECCION_BASIC_COLUMNS.map((c) => `${DIRECCION_TABLE}.${c}`);
    const depositoNombreColumns = DEPOSITO_NAME_COLUMNS.map((c) => `${DEPOSITO_TABLE}.${c}`);
    const parallelColumns = parallelTable != null ? parallelTable.columns.map((c) => `${parallelTable.name}.${c}`) : [];
    query = query.andWhereRaw(this._avoidInvalidsFilterWhereRaw()).select(
      ...mainColumns,
      ...pedidoClienteColumns,
      ...vendedorNombreColumns,
      ...transporteNombreColumns,
      ...
      ...depositoNombreColumns,
      ...parallelColumns
    ).leftOuterJoin(CLIENTE_TABLE, `${mainTable}.${CLIENTE_ID_FIELD}`, `${CLIENTE_TABLE}.${CLIENTE_ID_FIELD}`).leftOuterJoin(VENDEDOR_TABLE, `${mainTable}.${VENDEDOR_ID_FIELD}`, `${VENDEDOR_TABLE}.${VENDEDOR_ID_FIELD}`).leftOuterJoin(TRANSPORTE_TABLE, `${mainTable}.${TRANSPORTE_ID_FIELD}`, `${TRANSPORTE_TABLE}.${TRANSPORTE_ID_FIELD}`).leftOuterJoin(DIRECCION_TABLE, `${mainTable}.${DIRECCION_ID_FIELD}`, `${DIRECCION_TABLE}.${DIRECCION_ID_FIELD}`).leftOuterJoin(DEPOSITO_TABLE, `${mainTable}.${DEPOSITO_CODE_FIELD}`, `${DEPOSITO_TABLE}.${DEPOSITO_CODE_FIELD}`);
    if (parallelTable != null) {
      const { name } = parallelTable;
      query = query.leftOuterJoin(name, `${mainTable}.${PEDIDO_ID_FIELD}`, `${name}.${PEDIDO_ID_FIELD}`);
    }
    const data = await query;
    if (data == null) throw new DXTException(DXTErrorCode.NOT_FOUND);
    let transformedData = [];
    ...
    if (cacheKey != null) await this.cache.setMetadata(cacheKey, transformedData);
    const settings2 = await settingsProvider.getOrderSettings();
    const result = this._removePrivateFields(userRole, transformedData, settings2);
    return result;
  }
  _removePrivateFields(userRole, source, settings2) {
    if (userRole != UserRole.customer) return source;
    const includeShippingDate = settings2.show_shipping_date_to_customers;
    return source.map((order) => {
      ...
      return {
        ...remaining,
        fecha_entrega: includeShippingDate ? fecha_entrega : void 0
      };
    });
  }
  ...
  composeUpsertRecord(userRole, customer, orderAndRows) {
    const {
      id_lista,
      codigo_lista
    } = customer;
    ...
    const result = {
      ...
      [VENDEDOR_ID_FIELD]: customer.id_vendedor,
      [VENDEDOR_CODE_FIELD]: customer.codigo_vendedor,
  ...
  composeInsertRecord(userRole, customer, params) {
  ...
  composeUpdateRecord(userRole, customer, params) {
    const result = this.composeUpsertRecord(
      userRole,
      customer,
      params
  ...
  async create(userRole, customer, params) {
    const k = await this.getCompany();
    const { mainTable: orderTable } = this.config;
    const { rows } = params;
    const orderModel = this.composeInsertRecord(userRole, customer, params);
    const result = await k.transaction(async (trx) => {
      ...
      const createdOrder = await this.getOneOrNull(PEDIDO_CODE_FIELD, newOrderCode, { trx });
      if (createdOrder == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "PedidoBaseRepository.create -> createdOrder not found");
  ...
  async update(userRole, customer, params) {
    const k = await this.getCompany();
    const { mainTable: orderTable, mainIdField: orderIdField } = this.config;
    const { mainTable: rowsTable } = this._rowsRepository.config;
    const { rows, idPedido } = params;
    const orderModel = this.composeUpdateRecord(userRole, customer, params);
    const result = await k.transaction(
      async (trx) => {
        ...
        if (changedRecords != 1) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, `PedidoBaseRepository.update -> Invalid count of changed records: ${changedRecords}`);
        const updatedOrder = await this.getByIdOrNull(params.idPedido);
        if (updatedOrder == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "PedidoBaseRepository.update -> Updated record not found");
        if (updatedOrder.id == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "PedidoBaseRepository.update -> updatedOrder.id == null");
...
const renglonPedidoModelMapper = (m) => {
  const {
    ...
    TERMINAL_MODIFICACION_PRECIO: marcaPrecioEspecial
  } = m;
  const discount = isANumber(DESCUENTO) ? DESCUENTO : 0;
  const precio = applyTangoDiscount(PRECIO, discount);
  const cantidad = CANT_PEDID ?? 0;
  const es_precio_especial = marcaPrecioEspecial === TERMINAL_MODIFICACION_PRECIO_ESPECIAL;
  if (!isNotEmptyStr(codigo_articulo)) throw new DXTException(DXTErrorCode.TANGO_CLIENTE_DB_DATA_ERROR, `renglonPedidoModelMapper ${ARTICULO_CODE_FIELD}`);
  return {
    id_articulo,
    codigo_articulo,
    renglon: N_RENGLON ?? 0,
...
const renglonPedidoAndArticleModelMapper = (m) => {
  const r = renglonPedidoModelMapper(m);
  const {
    [ARTICULO_CODE_FIELD]: codigoArticulo,
    DESCRIPCIO,
    DESC_ADIC: descripcion_adicional
  } = m;
...
const renglonPedidoAndTaxesModelMapper = (m) => {
  const r = renglonPedidoAndArticleModelMapper(m);
  const {
    COD_IVA: codigo_iva,
    COD_S_IVA: codigo_sobre_iva,
    COD_II: codigo_ii,
    COD_S_II: codigo_sobre_ii
  } = m;
  return {
    ...r,
    codigo_iva,
...
class DXTArticuloPrintListRepository extends DXTArticuloListRepository {
  ...
  async getPackagePrintListMap() {
    ...
    const result = new Map(printListRaw.map((row, index) => {
      if (isANumber(row)) return [row, void 0];
...
class DXTArticuloEditListRepository extends DXTArticuloListRepository {
  ...
  async getByListIdWithTaxes(profileListId, additionalListId1, additionalDiscount1, additionalListId2, additionalDiscount2) {
    ...
    if (useArticleEditList) query = query.leftOuterJoin(ARTICULO_TABLE, fullDXTArticuloListCodeField, fullArticuloCodeField);
    ...
    if (bPricesQuery != null) query = query.leftOuterJoin(bPricesQuery, useArticleEditList ? fullDXTArticuloListCodeField : fullArticuloCodeField, `tpb.${ARTICULO_CODE_FIELD}`);
    if (cPricesQuery != null) query = query.leftOuterJoin(cPricesQuery, useArticleEditList ? fullDXTArticuloListCodeField : fullArticuloCodeField, `tpc.${ARTICULO_CODE_FIELD}`);
    let whereRawQuery = `tpa.${LISTA_ID_FIELD} = ?`;
    if (bListId != null) whereRawQuery += ` OR tpb.${LISTA_ID_FIELD} = ?`;
    if (cListId != null) whereRawQuery += ` OR tpc.${LISTA_ID_FIELD} = ?`;
    if (useArticleEditList) {
      whereRawQuery += ` OR (${fullDXTArticuloListCodeField} IS NOT NULL AND tpa.${LISTA_ID_FIELD} IS NULL AND `;
      if (bListId != null) whereRawQuery += `tpb.${LISTA_ID_FIELD} IS NULL AND `;
      if (cListId != null) whereRawQuery += `tpc.${LISTA_ID_FIELD} IS NULL AND `;
  ...
  async getHiddenArticlesMap() {
    ...
    const result = new Map(editListRaw.filter((row) => !isANumber(row)).map((row, index) => {
      if (isANumber(row)) return ["", void 0];
      ...
      if (articleCode == null) return ["", void 0];
...
class RenglonPedidoBaseRepository extends CompanyProvider {
  constructor(config2) {
    ...
    this.articuloColumns = renglonPedidoArticuloModelColumns.map((c) => `${ARTICULO_TABLE}.${c}`);
  }
  ...
  async _getByWhere(user, whereColumn, whereValue) {
    ...
    const rawData = await query.select(allColumns).leftOuterJoin(ARTICULO_TABLE, `${mainTable}.${ARTICULO_CODE_FIELD}`, `${ARTICULO_TABLE}.${ARTICULO_CODE_FIELD}`).innerJoin(pedidoTable, `${mainTable}.${PEDIDO_CODE_FIELD}`, `${pedidoTable}.${PEDIDO_CODE_FIELD}`).orderBy([PEDIDO_ID_FIELD, "N_RENGLON"]);
    if (rawData == null) throw new DXTException(DXTErrorCode.NOT_FOUND);
    ...
    if (idPedidoPrev != null) {
      result[idPedidoPrev] = transformedData.slice(sliceBegin, data.length);
    }
    await this.cache.setMetadata(cacheKey, result);
    return result;
  }
  ...
  toRenglonPedidoWithIdPedido(m) {
    ...
    if (id_pedido == null) throw new DXTException(DXTErrorCode.TANGO_DB_INVALID_DATA, `RenglonPedidoRepository.toRenglonWithIdPedido -> ${PEDIDO_ID_FIELD}`);
    return {
      ...renglonPedidoAndTaxesModelMapper(m),
      id_pedido
    };
  }
  toResultAndRelations(m) {
    return renglonPedidoAndArticleModelMapper(m);
  }
  ...
  composeInsertRecord(idPedido, numeroPedido, order, row, rowIndex) {
    ...
    const {
      ...
      quantity,
      precio,
      bonificacion_lista_de_precios,
      idMedidaStock,
      idMedidaStock2,
      idMedidaVentas,
      unidadMedidaSeleccionada
    } = row;
    const priceListDiscount = bonificacion_lista_de_precios;
    const adjustedPrice = invertedTangoDiscount(
      precio,
      priceListDiscount,
      orderDiscount
    );
    const result = {
      ...
      PRECIO_LISTA: adjustedPrice,
      DESCUENTO: 0,
      PRECIO_BONIF: adjustedPrice,
      TERMINAL_MODIFICACION_PRECIO: bonificacion_lista_de_precios != null ? TERMINAL_MODIFICACION_PRECIO_ESPECIAL : null,
      CANT_A_DES: quantity,
      CANT_A_FAC: quantity,
      CANT_PEDID: quantity,
      CANT_PEN_D: quantity,
...
const dxtPedidoExtraModelMapper = (m) => {
  const {
    [PEDIDO_ID_FIELD]: id,
    MENSAJE_INTERNO: mensaje_interno
...
class DXTPedidoExtraRepository extends TangoProvider {
  constructor() {
    super({
      mainTable: DXT_PEDIDO_EXTRA_TABLE,
      mainIdField: PEDIDO_ID_FIELD,
      columns: dxtPedidoExtraModelColumns
    });
  }
  async setInternalMessage(id_pedido, mensajeInterno) {
    const k = await this.getCompany();
    const { mainTable, mainIdField } = this.config;
    const changedRecords = await k(mainTable).where(mainIdField, id_pedido).update({
      MENSAJE_INTERNO: mensajeInterno
      // UPDATED_AT: new Date(),
    });
    if (changedRecords <= 0) {
      const k2 = await this.getCompany();
      await k2(mainTable).insert({
        [PEDIDO_ID_FIELD]: id_pedido,
        MENSAJE_INTERNO: mensajeInterno
        // UPDATED_AT: new Date(),
      });
  ...
  async deleteOrphanData() {
    try {
      const k = await this.getCompany();
      const { mainTable, mainIdField } = this.config;
      const query = `DELETE FROM ${mainTable} WHERE ${mainIdField} NOT IN (SELECT ${mainIdField} FROM ${PEDIDO_TABLE})`;
      await k.raw(query);
    } catch (e) {
      console.error(e);
    }
  ...
  toResult(m) {
    return dxtPedidoExtraModelMapper(m);
  }
}
const dxtPedidoExtraRepository = new DXTPedidoExtraRepository();
if (isDevelopment()) dxtPedidoExtraRepository.deleteOrphanData();
setInterval(() => {
  dxtPedidoExtraRepository.deleteOrphanData();
}, DAY_IN_MILLIS);
class PedidoRepository extends PedidoBaseRepository {
  constructor() {
    super({
      ...
      dependencies: [
        dxtPedidoExtraRepository
      ],
      parallelTable: {
        name: DXT_PEDIDO_EXTRA_TABLE,
        columns: dxtPedidoExtraModelColumns,
        writable: false
      }
    });
  ...
  composeUpsertRecord(userRole, customer, params) {
    const parentResult = super.composeUpsertRecord(userRole, customer, params);
    const {
      fechaHoraPedido,
      fechaEntrega
    } = params;
    const FECHA_PEDI = getDateOnly(fechaHoraPedido);
    const HORA = getDateHHMMSS(fechaHoraPedido);
    const FECHA_ENTR = getDateOnly(fechaEntrega);
    return {
      ...parentResult,
      FECHA_PEDI,
      HORA,
  ...
  composeInsertRecord(userRole, customer, params) {
    ...
    const FECHA_INGRESO = getDateOnly(fechaHoraIngreso);
    const HORA_INGRESO = getDateHHMMSS(fechaHoraIngreso);
    return {
      ...parentResult,
      FECHA_INGRESO,
      HORA_INGRESO
  ...
  composeUpdateRecord(userRole, customer, params) {
    ...
    const FECHA_ULTIMA_MODIFICACION = getDateOnly(fechaHoraModificacion);
    const HORA_ULTIMA_MODIFICACION = getDateHHMMSS(fechaHoraModificacion);
    return {
      ...parentResult,
      FECHA_ULTIMA_MODIFICACION,
      HORA_ULTIMA_MODIFICACION
  ...
  async create(userRole, customer, params) {
    ...
    const { mensaje_interno } = params;
    if (userRole != UserRole.customer && mensaje_interno != null) {
      await dxtPedidoExtraRepository.setInternalMessage(
        result.id,
        mensaje_interno
      );
    }
...
class DXTPedidoDraftRowRepository extends RenglonPedidoBaseRepository {
  ...
  // Actualiza los valores que pueden cambiar en el pedido, como los precios.
  async finalTransformer(user, row) {
    const {
      precio,
      ...remaining
...
async function getListas() {
  const preListas = await listaRepository.getAll();
  const listasById = new Map(preListas.map((row) => [row.id, row]));
  const listasByCode = new Map(preListas.map((row) => [row.codigo, row]));
  return { listasById, listasByCode };
}
async function getDirecciones(customerId) {
  const preDirecciones = await direccionRepository.getByCustomer(customerId);
  return preDirecciones.filter((direccion) => direccion.habilitada);
}
async function getTransportes() {
  return await transporteRepository.getAll();
}
async function getDepositos() {
  const preDepositos = await depositoRepository.getAll();
  return [
    {
      id: 0,
      code: "",
...
async function getCondiciones() {
  return condicionRepository.getAll();
}
async function getAsientos() {
  const preAsientos = await asientoRepository.getAll();
  return _.uniqWith(
    preAsientos.filter((asiento) => asiento.codigo_cuenta == 0),
    (a, b) => a.tipo == b.tipo
  );
}
async function getTalonarios() {
  const preTalonarios = await talonarioRepository.getAll();
  return preTalonarios.filter((talonario) => {
...
async function getAuxiliares(customerId) {
  ...
  if (transportes.length == 0) throw new DXTException(DXTErrorCode.EMPTY_TRANSPORTE_LIST);
  if (depositos.length == 0) throw new DXTException(DXTErrorCode.EMPTY_DEPOSITO_LIST);
  if (condiciones.length == 0) throw new DXTException(DXTErrorCode.EMPTY_CONDICION_LIST);
  if (asientos.length == 0) throw new DXTException(DXTErrorCode.EMPTY_ASIENTO_LIST);
  if (talonarios.length == 0) throw new DXTException(DXTErrorCode.EMPTY_TALONARIO_LIST);
  if (direcciones.length == 0) throw new DXTException(DXTErrorCode.EMPTY_DIRECCION_LIST);
  return {
    ...listas,
    transportes,
    depositos,
...
function getPropertyIfSameOrNull(data, propertyName) {
  ...
  for (let i = 1; i < l; i++) {
    if (data[i][propertyName] != first) return null;
  }
  return first;
}
async function getOptimalBillingProfile(user, customer) {
  ...
  const customerCondicionAvailable = isAnInteger(customer.codigo_condicion) && customer.codigo_condicion > 0;
  const customerTransporteAvailable = typeof customer.codigo_transporte === "string" && customer.codigo_transporte.trim() != "";
  const customerListaAvailable = isAnInteger(customer.codigo_lista) && customer.codigo_lista > 0;
  const { show_shipping_date_to_customers } = await settingsProvider.getOrderSettings();
  const optimalBillingProfile = {
    ...
    codigo_condicion_venta: customerCondicionAvailable ? customer.codigo_condicion : cbp?.codigo_condicion_venta ?? ubp.codigo_condicion_venta,
    codigo_transporte: customerTransporteAvailable ? customer.codigo_transporte : cbp?.codigo_transporte ?? ubp.codigo_transporte,
    codigo_lista_precio: customerListaAvailable ? customer.codigo_lista : cbp?.codigo_lista_precio ?? ubp.codigo_lista_precio,
    fecha_alta_visible: ubp.fecha_alta_visible,
    fecha_entrega_visible: (!user.isCustomer() || show_shipping_date_to_customers) && ubp.fecha_entrega_visible
  };
  if (customer.discrimina_iva) return optimalBillingProfile;
  const talonarios = await getTalonarios();
  const selectedTalonario = talonarios.find((talonario) => talonario.code == optimalBillingProfile.codigo_talonario_factura);
  if (selectedTalonario != null && _talonarioConsumidorFinal(selectedTalonario)) return optimalBillingProfile;
  const talonarioConsumidorFinal = talonarios.find((talonario) => _talonarioConsumidorFinal(talonario));
  if (!talonarioConsumidorFinal) throw new DXTException(DXTErrorCode.INVALID_CONSUMIDOR_FINAL_TALONARIO);
  return {
...
class DXTPedidoDraftRepository extends PedidoBaseRepository {
  constructor(config2) {
    super({
      ...config2,
      columns: dxtPedidoDraftModelColumns
    });
  }
  toResult(m) {
  ...
  async descriptionExists(user, description, draftIdToIgnore) {
    if (user.isAdmin()) throw new DXTException(DXTErrorCode.CUSTOMER_OR_SELLER_ROLE_REQUIRED);
    const isCustomer = user.isCustomer();
    const column = isCustomer ? CLIENTE_ID_FIELD : VENDEDOR_ID_FIELD;
    const tangoId = user.tangoId?.valueOf();
    if (tangoId == null) throw new DXTException(isCustomer ? DXTErrorCode.TANGO_CUSTOMER_NO_LONGER_EXISTS : DXTErrorCode.TANGO_SELLER_NO_LONGER_EXISTS);
    ...
    if (draftIdToIgnore != null && data?.id === draftIdToIgnore) return false;
    return data != null;
  }
  composeUpsertRecord(userRole, customer, params) {
    const parentResult = super.composeUpsertRecord(userRole, customer, params);
    ...
    const horaIngreso = getDateHHMMSS(fechaHoraIngreso);
    const fechaIngreso = getDateOnly(fechaHoraIngreso);
    return {
      ...parentResult,
      [PEDIDO_DESCRIPTION_FIELD]: descripcion,
      ...
      HORA: horaIngreso,
      MENSAJE_INTERNO: userRole != UserRole.customer && mensaje_interno != null ? mensaje_interno : void 0
    };
  }
  composeInsertRecord(userRole, customer, params) {
    const parentResult = this.composeUpsertRecord(
  ...
  async setFavorite(orderId, isFavorite) {
    ...
    if (result < 1) throw new DXTException(DXTErrorCode.DRAFT_NOT_FOUND);
  }
}
...
function _isCustomer(user) {
  const role = user.role.valueOf();
  if (role == UserRole.admin) throw new DXTException(DXTErrorCode.CUSTOMER_OR_SELLER_ROLE_REQUIRED);
  return role == UserRole.customer;
}
async function getOrder(user, idPedido, isDraft, adminAllowed, includeRows) {
  ...
  const rows = includeRows ? isDraft ? await buyListRowRepository.getByIdPedido(user, idPedido) : await buyListRowRepository.getByNumeroPedido(user, header.numero_pedido) : null;
  const rowsOk = rows != null || !includeRows;
  if (header == null || !rowsOk) throw new DXTException(isDraft ? DXTErrorCode.DRAFT_NOT_FOUND : DXTErrorCode.ORDER_NOT_FOUND);
...
async function _checkUserIsOwnerOrThrow(user, orderHeader, adminAllowed) {
  const { role } = user;
  if (role.isAdmin()) {
    if (!adminAllowed) throw new DXTException(DXTErrorCode.CUSTOMER_OR_SELLER_ROLE_REQUIRED);
    return;
  }
  if (role.isSeller()) {
    ...
    if (!isSellerCustomer) throw new DXTException(DXTErrorCode.ORDER_CUSTOMER_DOES_NOT_BELONGS_TO_SELLER);
    return;
  }
  const idCliente = user.tangoId.valueOf();
  if (orderHeader.id_cliente !== idCliente) throw new DXTException(DXTErrorCode.ORDER_DOES_NOT_BELONGS_TO_CUSTOMER);
}
async function getArticlesWithTaxes(user, customer, alwaysIncludeArticlesWithoutPrice, splitInGroups) {
  ...
  if (user.isSeller()) {
    ...
    if (additionalListId1 != null) additionalDiscount1 = user.bonificacionListaAdicional?.valueOf();
  ...
  const listId = customer.id_lista;
  const keepArticlesWithoutPrice = alwaysIncludeArticlesWithoutPrice || user.verSinPrecio.valueOf();
  const {
    article_edit_list_group_separator,
    article_edit_list_enabled
  } = await settingsProvider.getOrderSettings();
  ...
  const useArticleEditList = articleEditListExists && article_edit_list_enabled;
  const groupSeparator = articleEditListExists ? ArticleGroupSeparator.none : article_edit_list_group_separator;
  const cacheKey = `groupedArticles_${listId}_${additionalListId1 ?? "0"}_${additionalListId2 ?? "0"}_${useArticleEditList ? "OL" : "NOL"}_${splitInGroups ? "S" : "NS"}_${groupSeparator}`;
  ...
  if (useArticleEditList && splitInGroups) {
    result = _splitByOrderListGroup(data);
  } else {
    const onlyFullArticles = data.filter((item) => typeof item !== "string");
    result = splitInGroups ? _splitByArticleCode(groupSeparator, onlyFullArticles) : { [ARTICLE_GROUP_NO_NAME]: onlyFullArticles };
  }
...
function _splitByOrderListGroup(data) {
  ...
  data.forEach((item) => {
    ...
    if (isGroupLabel) {
      ...
      if (lastGroupName.length == 0) lastGroupName = ARTICLE_GROUP_NO_NAME;
...
function _splitByArticleCode(groupSeparator, data) {
  let result = {};
  let currentList = null;
  let lastGroupName = null;
  let totalGroups = 0;
  let maxGroupLength = 1;
  const articlesWithCodeGroups = data.filter((articulo) => articulo.id_lista != null).map((articulo) => {
    const { codigo, id_lista } = articulo;
    const groups = _getGroups(groupSeparator, codigo.trim());
    if (id_lista != null) {
  ...
  articlesWithCodeGroups.forEach((articulo) => {
    const { groups } = articulo;
    const possibleGroupName = groups.slice(0, groupNameLimit).join("");
    const currentGroupName = maxGroupLength > 1 && possibleGroupName.length > 0 ? possibleGroupName : ARTICLE_GROUP_NO_NAME;
...
function _getGroups(groupSeparator, code) {
  code = code.trim().toUpperCase();
  if (groupSeparator == ArticleGroupSeparator.none) return [code];
  if (groupSeparator == ArticleGroupSeparator.space) {
    const groupsBySpace = code.split(" ");
    return groupsBySpace.map((g, i) => i == 0 ? g : ` ${g}`);
  }
  if (groupSeparator == ArticleGroupSeparator.dash) {
    const groupsByDash = code.split("-");
    return groupsByDash.map((g, i) => i == 0 ? g : `-${g}`);
  }
  if (groupSeparator == ArticleGroupSeparator.underscore) {
    const groupsByUnderscore = code.split("_");
    return groupsByUnderscore.map((g, i) => i == 0 ? g : `_${g}`);
  }
  if (groupSeparator == ArticleGroupSeparator.lettersAndNumbers) {
    const groupByLettersAndNumbers = _splitLettersAndNumbers(code);
    return groupByLettersAndNumbers;
  }
  return [code];
}
function _splitLettersAndNumbers(input) {
  let result = [];
  let preIsNumber = null;
  let currentGroup = "";
  for (let i = 0; i < input.length; i++) {
    let char = input[i];
    const isNumber = char >= "0" && char <= "9";
    if (preIsNumber == null || preIsNumber !== isNumber) {
      if (currentGroup.length > 0) result.push(currentGroup);
      currentGroup = char;
      preIsNumber = isNumber;
    } else {
      currentGroup += char;
    }
  }
...
function percentageToFactor(value) {
  ...
  return (1 + Math.abs(value) / 100) * sign;
}
function adjustPrice(params, article, listasById, alicuotas, forceTaxesElimination) {
  const { id_lista, codigo_iva, codigo_ii, codigo_sobre_iva, codigo_sobre_ii, precio } = article;
  if (id_lista == null || precio == null) return null;
  const { usuarioAplicaSobreIVA, usuarioAplicaSobreII } = params;
  const lista = id_lista != null ? listasById.get(id_lista) : null;
  ...
  const eliminarIVA = forceTaxesElimination ? true : params.usuarioExentoIVA;
  const eliminarII = forceTaxesElimination ? true : params.usuarioExentoII;
  ...
  let result = precio;
  if (!listaIncluyeIVA && !eliminarIVA) {
    result = result * iva;
    if (usuarioAplicaSobreIVA) {
      result = result * sobreIVA;
    }
  } else if (listaIncluyeIVA && eliminarIVA) {
    result = result / iva;
    if (usuarioAplicaSobreIVA) {
      result = result / sobreIVA;
    }
  }
  if (!listaIncluyeII && !eliminarII) {
    result = result * impuestoInterno;
    if (usuarioAplicaSobreII) {
      result = result * sobreII;
    }
  } else if (listaIncluyeII && eliminarII) {
    result = result / impuestoInterno;
    if (usuarioAplicaSobreII) {
      result = result / sobreII;
    }
...
function _getRowsWithUnknownPrices(articles, rows) {
  ...
  rows.forEach((row) => {
    ...
    if (isOrphan) result.push(row);
  });
  return result;
}
function _addQuantitiesAndAdjustPrices(orderHeader, articles, alicuotas, listasById, rows) {
  ...
  const commonAdjustPriceParams = {
    ...
    usuarioExentoIVA: orderHeader.usuario_exento_iva,
    usuarioExentoII: orderHeader.usuario_exento_ii
  };
  return Object.fromEntries(
    Object.entries(articles).map(
      ([groupName, articles2]) => {
...
async function prepareOrderHeader(user, originalOrder, customer, billingProfile, auxiliares, createNew, startDraft) {
  if (!createNew && originalOrder == null) throw new DXTException(DXTErrorCode.INTERNAL_SERVER_ERROR, "prepareOrderHeader() -> createNew must be true if originalOrder is null");
  const originalOrderIsDraft = originalOrder?.isDraft === true;
  const sameType = originalOrderIsDraft == startDraft;
  if (!createNew && !sameType) throw new DXTException(DXTErrorCode.INTERNAL_SERVER_ERROR, "prepareOrderHeader() -> When createNew is false, both originalOrder.isDraft and startDraft must be the same");
  const sellerId = user.vendedorId?.valueOf();
  if (sellerId == null) throw new DXTException(DXTErrorCode.INTERNAL_SERVER_ERROR, "prepareOrderHeader() -> sellerId is null");
  ...
  const bonificacion_editable = user.isSeller() && billingProfile.bonificacion_editable;
  const descripcion = startDraft && originalOrder != null ? originalOrder.header.descripcion : "";
  const leyenda1 = originalOrder != null ? originalOrder.header.leyenda1 : "";
  const leyenda2 = originalOrder != null ? originalOrder.header.leyenda2 : "";
  const leyenda3 = originalOrder != null ? originalOrder.header.leyenda3 : "";
  const leyenda4 = originalOrder != null ? originalOrder.header.leyenda4 : "";
  const leyenda5 = originalOrder != null ? originalOrder.header.leyenda5 : "";
  const orginalInternalMessage = originalOrder?.header.mensaje_interno;
  const mensaje_interno = (user.role.valueOf() != UserRole.customer ? orginalInternalMessage : null) ?? null;
  ...
  const {
    ...
    direccion_editable,
    fecha_alta_editable,
    fecha_entrega_editable,
    fecha_alta_visible,
    fecha_entrega_visible,
    talonario_visible,
    asiento_visible,
    transporte_visible,
    condicion_visible,
  ...
  const originalShippingDate = originalOrderIsDraft ? null : originalOrder?.header.fecha_entrega;
  const creationDate = (createNew || originalOrderIsDraft ? /* @__PURE__ */ new Date() : originalOrder?.header.fecha_hora_alta) ?? /* @__PURE__ */ new Date();
  const shippingDate = createNew || originalShippingDate == null ? addDays(creationDate, user.diaDeEntrega.valueOf()) : originalShippingDate;
  const isFavorite = !createNew && originalOrderIsDraft ? originalOrder.header.es_favorito : false;
  ...
  const lista = listasByCode.get(codigo_lista_precio);
  if (lista == null) throw new DXTException(DXTErrorCode.INTERNAL_SERVER_ERROR, "prepareOrderHeader() -> Lista de precios inexistente");
  const result = {
    ...
    id_vendedor: customer.id_vendedor,
    ...
    fecha_alta_editable,
    fecha_entrega: dateToISOStringZ(shippingDate),
    fecha_entrega_editable,
    ...
    asientos: _setSelectedItem(asientos, asiento_editable, DXTErrorCode.UNKNOWN_ASIENTO, codigo_asiento, (el) => codigo_asiento.trim() == "" || el.code == codigo_asiento),
    ...
    usuario_exento_iva: exento_iva,
    usuario_exento_ii: exento_ii,
    usuario_aplica_sobre_iva: aplica_sobre_iva,
    usuario_aplica_sobre_ii: aplica_sobre_ii
  };
  return result;
}
function _setSelectedItem(list, editable, unknownElementError, targetValue, checkSelected) {
  if (targetValue == null) return list;
  if (editable) {
    return list.map((el, index) => ({
      ...el,
      selected: checkSelected?.(el, index) === true ? true : void 0
    }));
  }
  if (checkSelected == null) throw new DXTException(DXTErrorCode.INTERNAL_SERVER_ERROR, "_setSelectedItem -> <checkSelected> must be defined when <editable> is false");
  let result = list.filter((el, index) => checkSelected(el, index));
  if (result.length == 0) result = list.slice(1);
  if (result.length == 0) throw new DXTException(unknownElementError, valueToString(targetValue));
  return result;
}
...
async function startPedido(user, params) {
  ...
  if (!startDraft && createNew && !user.puedeCrearPedido.valueOf()) throw new DXTException(DXTErrorCode.USER_CANNOT_CREATE_ORDERS);
  if (!startDraft && !createNew && !user.puedeEditarPedido.valueOf()) throw new DXTException(DXTErrorCode.USER_CANNOT_UPDATE_ORDERS);
  const originalOrder = mode2 == 1 ? null : await getOrder(user, params.idPedido, params.isDraft, false, true);
...
class SellerRootController extends AuthenticatedRootController {
  async onRequest(req) {
    await super.onRequest(req);
    if (!req.auth.user.role.isSeller()) throw new DXTException(DXTErrorCode.FORBIDDEN);
  }
}
...
async function _startNew(user, createDraft) {
  ...
  if (tangoId == null || !role.isCustomer()) throw new DXTException(DXTErrorCode.CUSTOMER_ROLE_REQUIRED);
...
async function _startNewForCustomer(user, idCliente, createDraft) {
  const { tangoId, role } = user;
  if (tangoId == null || !role.isSeller()) throw new DXTException(DXTErrorCode.SELLER_ROLE_REQUIRED);
  return await startPedido(user, {
    mode: StartPedidoMode.create,
    idCliente,
    createDraft
...
async function readDir(directory, options) {
  ...
  if (!(await fs.promises.stat(directory)).isDirectory())
    throw new NotDirectoryError(directory);
  const files = await fs.promises.readdir(directory);
  for (const file of files) {
    if (file === "." || file === "..") break;
    const fullname = path.join(directory, file);
    const isDirectory = (await fs.promises.stat(fullname)).isDirectory();
    if ((!isDirectory || includeDirs) && (!filter || filter(file, directory)))
      result.push(fullname);
    if (isDirectory && recursive) {
      result.push(...await readDir(fullname, options));
    }
  }
  return result;
}
...
class EmailProvider {
  constructor() {
    ...
    this._initializingTransporter = false;
    this._initTransporter();
    eventBus.on(
      MiscSettingsChangedEvent.name,
      this._onMiscSettingsChanged.bind(this)
    );
  }
  _onMiscSettingsChanged(event) {
    (async () => {
      await this._initTransporter();
    })();
  }
  async _initTransporter() {
    if (this._initializingTransporter) return;
    this._initializingTransporter = true;
    try {
      await this._loadEmailTemplates();
      const settings2 = await settingsProvider.getMiscSettings();
      const {
        smtp_enabled,
        smtp_host,
  ...
  async _loadEmailTemplates() {
    const newTemplates = /* @__PURE__ */ new Map();
    const emailsPath = path$2.join(process.cwd(), "emails");
    const files = await readDir(emailsPath, {
      recursive: false,
      filter: (filename) => {
        const extension = getFileExtension(filename);
        return extension === MJML_EXTENSION;
      }
    });
    for (const file of files) {
      const templateName = path$2.basename(file, MJML_EXTENSION);
      const mjmlData = (await readFile$1(file)).toString();
      const mjmlResult = mjml2html(mjmlData, {});
      if (mjmlResult.errors.length > 0) {
        throw `MJML compilation error on ${file}`;
      }
      newTemplates.set(templateName, mjmlResult.html);
    }
    this._templates = newTemplates;
  }
  async sendEmail(from, to, cc, subject, templateName, values) {
    if (this._transporter == null) return;
    const template = this._templates?.get(templateName);
    let renderedSubject;
    let renderedBody;
    try {
      renderedSubject = mustache.render(subject, values ?? {});
      renderedBody = template != null ? mustache.render(template, values ?? {}) : values != null ? stringifyJSON(values, void 0, 4) : "BODY";
    } catch (e) {
      console.error(e);
    }
    if (renderedSubject == null || renderedBody == null) return;
    let mailOptions = {
      from: from.toString(),
      to: to?.toString(),
      cc: cc != null && cc.length > 0 ? cc.map((e) => e.toString()) : void 0,
      subject: renderedSubject,
      html: renderedBody,
      text: "Utilice un visor HTML para ver este correo"
    };
    try {
...
var EmailOrderAction = /* @__PURE__ */ ((EmailOrderAction2) => {
  EmailOrderAction2[EmailOrderAction2["cancel"] = 0] = "cancel";
  EmailOrderAction2[EmailOrderAction2["delete"] = 1] = "delete";
  EmailOrderAction2[EmailOrderAction2["create"] = 2] = "create";
  EmailOrderAction2[EmailOrderAction2["update"] = 3] = "update";
  return EmailOrderAction2;
})(EmailOrderAction || {});
const ORDER_DELETED_SUBJECT = "Se ha eliminado el pedido {{{orderCode}}}";
const ORDER_CANCELLED_SUBJECT = "Se ha anulado el pedido {{{orderCode}}}";
const ORDER_CREATED_SUBJECT = "Nuevo pedido {{{orderCode}}} para {{{customerName}}} {{{subjectShippingDate}}}";
const ORDER_UPDATED_SUBJECT = "Se ha modificado el pedido {{{orderCode}}} para {{{customerName}}} {{{subjectShippingDate}}}";
...
function formatArticleName(nombre, descriptionAdicional) {
  if (nombre == null) return NO_NAME;
  let result = nombre.trim();
  if (descriptionAdicional != null && descriptionAdicional.trim().length > 0) result = `${result} (${descriptionAdicional.trim()})`;
  return result;
}
function fullArticleDescription(nombre, codigo_articulo, descriptionAdicional) {
  if (codigo_articulo == null) return NONEXISTENT_PRODUCT;
  if (nombre == null) return NO_NAME;
  return `${codigo_articulo} - ${formatArticleName(nombre, descriptionAdicional)}`;
}
function formatAuxiliares(codigo, nombre, gender) {
  if (codigo != null && nombre != null) return `${codigo} - ${nombre}`;
  return gender === "f" ? NONE_F : NONE_M;
}
function formatClienteVendedor(codigo, nombre) {
  return `${codigo} - ${nombre}`;
}
const calculateTotals = (quantities, prices, orderDiscount) => {
  let total = 0;
  quantities.forEach((quantity, id) => {
    const { amount, discount } = prices[id];
    if (amount == null) return;
    const finalDiscount = discount ?? orderDiscount;
    const subtotal = applyTangoDiscount(quantity * amount, finalDiscount);
    total += subtotal ?? 0;
  });
  return total;
};
function formatTaxesLabel(exentoIVA, exentoII) {
  if (exentoIVA) {
    return exentoII ? "sin impuestos" : "sin IVA";
  }
  return exentoII ? "con IVA" : "impuestos incluídos";
}
...
async function dispatchOrderEmail(user, orderAction, from, to, bcc, order) {
  const { company_name: companyName } = await settingsProvider.getProgramConfig();
  const {
    show_shipping_date_to_customers,
    order_create_update_message
  } = await settingsProvider.getOrderSettings();
  const orderCreateUpdateMessage = order_create_update_message.length > 0 ? order_create_update_message : null;
  const { header, rows } = order;
  const orderDeleted = orderAction == EmailOrderAction.delete;
  const orderCancelled = orderAction == EmailOrderAction.cancel;
  const orderDeletedOrCancelled = orderDeleted || orderCancelled;
  const orderCreated = orderAction == EmailOrderAction.create;
  const orderUpdated = orderAction == EmailOrderAction.update;
  const orderCreatedOrUpdated = orderCreated || orderUpdated;
  const {
    descuento: discount,
    leyenda1,
    leyenda2,
  ...
  const taxesLabel = formatTaxesLabel(exemptIVA, exemptII).toLocaleUpperCase();
  const captions = orderCaptionsToArray(
    leyenda1,
    leyenda2,
    leyenda3,
    leyenda4,
  ...
  const total = rows.reduce((total2, row) => total2 + (row.bonifiedPrice ?? 0) * row.quantity, 0);
  const shippingDate = show_shipping_date_to_customers ? dateToLocale(header.fecha_entrega) : null;
  const values = {
    orderCreateUpdateMessage,
    companyName,
    orderCreated,
    orderUpdated,
    orderDeleted,
    orderCancelled,
    orderCreatedOrUpdated,
    orderDeletedOrCancelled,
    isAdministrator: user.isAdmin(),
    isCustomer: user.isCustomer(),
    isSeller: user.isSeller(),
    orderCode: header.numero_pedido,
    customerCode: header.codigo_cliente,
    customerName,
    sellerCode: header.codigo_vendedor,
    sellerName,
    inputDatetime: datetimeToLocale(header.fecha_hora_ingreso),
    shippingDate,
    subjectShippingDate: shippingDate != null ? `(entrega ${shippingDate}` : "",
    discount,
    thereAreCaptions: captions.length > 0,
    captions,
    ...
    articles: rows.map((row) => {
      return {
        ...row,
        bonifiedPrice: formatPrice(row.bonifiedPrice),
        subtotalWithDiscount: formatPrice(row.subtotalWithDiscount)
      };
    })
  };
  let subject;
  switch (orderAction) {
    case EmailOrderAction.cancel:
      subject = ORDER_CANCELLED_SUBJECT;
      break;
    case EmailOrderAction.delete:
      subject = ORDER_DELETED_SUBJECT;
      break;
    case EmailOrderAction.create:
      subject = ORDER_CREATED_SUBJECT;
      break;
    case EmailOrderAction.update:
      subject = ORDER_UPDATED_SUBJECT;
      break;
  }
...
async function resolveTargetEmails(user, adminEmail, order) {
  user.id.valueOf();
  const {
    id_cliente,
    id_vendedor
  } = order.header;
  const sellersEmails = await dxtVendedorRepository.getEmailsByTangoId(id_vendedor);
  const customersEmails = await dxtClienteRepository.getEmailsByTangoId(id_cliente);
  const to = adminEmail;
  const cc = _.uniq([
    ...sellersEmails.map((r) => r.email),
    ...customersEmails.map((r) => r.email)
  ]).map((e) => new VOEmail(e));
  const anyEmail = to != null || cc.length > 0;
  if (!anyEmail) return null;
  return { to, cc };
}
function applyTaxesToPrice(params, article, alicuotas) {
  const { codigo_iva, codigo_ii, codigo_sobre_iva, codigo_sobre_ii, precio } = article;
  if (precio == null) return null;
  const { usuarioAplicaSobreIVA, usuarioAplicaSobreII } = params;
  const agregarIVA = !params.usuarioExentoIVA;
  const agregarII = !params.usuarioExentoII;
  let result = precio;
  if (agregarIVA) {
    const ivaPercentage = codigo_iva != null ? alicuotas.get(codigo_iva) ?? 0 : 0;
    const sobreIVABruto = codigo_sobre_iva != null ? alicuotas.get(codigo_sobre_iva) ?? 0 : 0;
...
function removeTaxesFields(row) {
  const {
    codigo_iva,
    codigo_sobre_iva,
    codigo_ii,
...
function applyTaxesToOrder(header, rows, alicuotas) {
  const applyTaxesToPriceParams = {
    usuarioAplicaSobreIVA: header.cliente_aplica_sobre_iva,
    usuarioAplicaSobreII: header.cliente_aplica_sobre_ii,
    usuarioExentoIVA: header.cliente_exento_iva,
    usuarioExentoII: header.cliente_exento_ii
  };
  const newRows = rows.map((row) => {
    const priceWithTaxes = applyTaxesToPrice(
      applyTaxesToPriceParams,
      row,
      alicuotas
    );
    const renglon = removeTaxesFields(row);
    return {
      ...renglon,
      precio: priceWithTaxes
      // id_pedido, 
    };
  });
  return newRows;
}
async function prepareEmailOrderData(options) {
  const {
    action: action2,
    user
  } = options;
  const alicuotas = await alicuotaRepository.getAllPercentages();
  const cancelOrDelete = action2 == EmailOrderAction.cancel || action2 == EmailOrderAction.delete;
  const orderHeader = cancelOrDelete ? options.order.header : options.orderHeader;
  const orderRows = cancelOrDelete ? options.order.rows : await renglonPedidoRepository.getByIdPedido(user, orderHeader.id);
  const customer = cancelOrDelete ? await clienteRepository.getExtendedById(orderHeader.id_cliente) : options.customer;
  const { id_cliente, id_vendedor, descuento } = orderHeader;
  const tangoCustomer = await clienteRepository.getById(id_cliente);
  const tangoSeller = await vendedorRepository.getById(id_vendedor);
  const {
    aplica_sobre_iva,
    aplica_sobre_ii,
    exento_iva,
  ...
  const newRows = applyTaxesToOrder(
    {
      ...orderHeader,
      cliente_aplica_sobre_iva: aplica_sobre_iva,
      cliente_aplica_sobre_ii: aplica_sobre_ii,
      cliente_exento_iva: exento_iva,
      cliente_exento_ii: exento_ii
    },
    orderRows,
    alicuotas
  );
  let totalWithoutDiscount = 0;
  const resultRows = newRows.map((r) => {
    const price = r.precio;
    const bonifiedPrice = applyTangoDiscount(price, descuento);
    const subtotal = price != null ? price * r.cantidad : null;
    const subtotalWithDiscount = bonifiedPrice != null ? bonifiedPrice * r.cantidad : null;
    totalWithoutDiscount += subtotal ?? 0;
    return {
      code: r.codigo_articulo,
      name: formatArticleName(r.nombre_articulo, r.descripcion_adicional),
      bonifiedPrice,
      isSpecialPrice: r.es_precio_especial,
      quantity: r.cantidad,
  ...
  const resultHeader = {
    ...orderHeader,
    customerName: tangoCustomer.screen_name,
    sellerName: tangoSeller.screen_name,
    totalWithoutDiscount,
    exemptIVA: customer.exento_iva,
    exemptII: customer.exento_ii
  };
  return {
    header: resultHeader,
    rows: resultRows
...
async function sendOrderEmail(options) {
  ...
  const adminEmail = admin_email != null ? new VOEmail(admin_email, USER_ROLE_ADMIN) : null;
  const emailOrderData = await prepareEmailOrderData(options);
  const targetEmails = await resolveTargetEmails(
    options.user,
    adminEmail,
    emailOrderData
  );
  if (targetEmails == null) return;
  const fromEmailAddress = smtp_from_email ?? admin_email ?? DEFAULT_SMTP_FROM_EMAIL;
  const fromEmail = new VOEmail(fromEmailAddress, company_name);
...
async function createOrUpdateOrder(user, customer, input) {
  const repository = input.saveDraft ? getDraftRepository(user) : pedidoRepository;
  const { order_create_update_message } = await settingsProvider.getOrderSettings();
  const isUpdating = input.idPedido != null;
  const userRole = user.role.valueOf();
  const orderHeader = isUpdating ? await repository.update(userRole, customer, input) : await repository.create(userRole, customer, input);
  const result = {
    nuevo_pedido: false,
    id_pedido: orderHeader.id,
    numero_pedido: orderHeader.numero_pedido,
    message_to_user: order_create_update_message.length > 0 ? order_create_update_message : void 0
  };
  if (input.saveDraft) return result;
  if (isUpdating) {
    sendOrderEmail({
      action: EmailOrderAction.update,
      user,
      customer,
      orderHeader
    });
    ...
    sendOrderEmail({
      action: EmailOrderAction.create,
      user,
      customer,
      orderHeader
    });
...
async function validateInputRows(user, originalOrder, customer, billingProfile, auxiliares, validatedOrderHeader, inputRows) {
  ...
  const lista = listasById.get(idListaDePrecio);
  if (lista == null) throw new DXTException(DXTErrorCode.UNEXPECTED_ERROR, "validateInputRows -> lista == null");
  const { incluye_iva, incluye_ii } = lista;
  const adjustPriceParams = {
    // bonificacion: bonificacion,
    listaIncluyeIVA: incluye_iva,
    listaIncluyeII: incluye_ii,
    usuarioAplicaSobreIVA: customer.aplica_sobre_iva,
    usuarioAplicaSobreII: customer.aplica_sobre_ii,
    usuarioExentoIVA: customer.exento_iva,
    usuarioExentoII: customer.exento_ii
  };
...
function _validateOrderRow(row, article) {
  ...
  if (article == null) throw new DXTException(DXTErrorCode.INACCESIBLE_ORDER_ROW_ARTICLE);
  const {
    id,
    codigo,
    nombre,
  ...
  return {
    ...
    codeArticle: codigo,
    nameArticle: nombre,
    additionalDescription: descripcion_adicional,
    quantity,
    id_lista,
    precio,
    bonificacion_lista_de_precios,
...
function _calculateTotal(rows, orderDiscount) {
  let total = 0;
  rows.forEach((row) => {
    const discount = row.bonificacion_lista_de_precios ?? orderDiscount;
    const subtotal = applyTangoDiscount(row.quantity * (row.precio ?? 0), discount);
    total += subtotal ?? 0;
  });
  return total;
}
async function validateInputHeader(user, originalOrder, customer, params, billingProfile, auxiliares) {
  ...
  if (saveDraft) {
    ...
    if (descriptionExists) throw new DXTException(DXTErrorCode.DUPLICATED_DRAFT_DESCRIPTION);
  }
  const {
    leyenda1,
    leyenda2,
  ...
  const fechaAlta = fecha_alta_editable ? new Date(fecha_alta) : /* @__PURE__ */ new Date();
  const fechaEntrega = fecha_entrega_editable ? new Date(fecha_entrega) : addDays(fechaAlta, user.diaDeEntrega.valueOf());
  const validatedOrderHeader = {
    ...
    fechaAlta,
    fechaEntrega,
    mensaje_interno: user.role.valueOf() != UserRole.customer ? mensaje_interno : null,
...
function _checkSelection(list, isEditable, selectedId, errorCode) {
  ...
  if (index < 0) throw new DXTException(errorCode);
  return [
    list[index].id,
    list[index].code
  ];
...
async function validateInputParams(user, originalOrder, customer, params) {
  ...
  if (saveDraft) {
    ...
    const fechaHoraIngreso2 = /* @__PURE__ */ new Date();
    return {
      ...validatedOrder,
      saveDraft,
      idPedido,
      descripcion,
      esFavorito,
      fechaHoraIngreso: fechaHoraIngreso2
    };
  }
  const realOrder = originalOrder?.isDraft === false ? originalOrder : null;
  const fechaHoraIngreso = realOrder?.header.fecha_hora_ingreso ?? /* @__PURE__ */ new Date();
  const fechaHoraPedido = realOrder?.header.fecha_hora_alta ?? validatedOrder.fechaAlta;
  const fechaEntrega = realOrder?.header.fecha_entrega ?? validatedOrder.fechaEntrega;
  const fechaHoraModificacion = /* @__PURE__ */ new Date();
  return {
    ...validatedOrder,
    saveDraft,
    idPedido,
...
async function savePedido(user, params) {
  ...
  const createNew = mode2 == SaveOrderMode.create;
  if (user.isAdmin()) throw new DXTException(DXTErrorCode.CUSTOMER_OR_SELLER_ROLE_REQUIRED);
  if (!saveDraft && createNew && !user.puedeCrearPedido.valueOf()) throw new DXTException(DXTErrorCode.USER_CANNOT_CREATE_ORDERS);
  if (!saveDraft && !createNew && !user.puedeEditarPedido.valueOf()) throw new DXTException(DXTErrorCode.USER_CANNOT_UPDATE_ORDERS);
  const originalIsDraft = mode2 == SaveOrderMode.update && saveDraft;
  const originalOrder = mode2 == SaveOrderMode.update ? await getOrder(user, input.id_pedido, originalIsDraft, false, true) : null;
  if (!saveDraft && !createNew && originalOrder.header.estado != EstadoPedido.INGRESADO) {
    throw new DXTException(DXTErrorCode.USER_CANNOT_UPDATE_ORDER_IN_CURRENT_STATUS);
  }
  ...
  const idCustomerVendedor = customer.id_vendedor;
  if (user.isSeller() && idCustomerVendedor != user.vendedorId?.valueOf()) throw new DXTException(DXTErrorCode.CUSTOMER_DOES_NOT_BELONGS_TO_SELLER);
...
async function cancelPedido(user, idPedido) {
  ...
  if (!user.puedeAnularPedido.valueOf()) {
    if (deleteOrder) throw new DXTException(DXTErrorCode.USER_CANNOT_DELETE_ORDERS);
    throw new DXTException(DXTErrorCode.USER_CANNOT_CANCEL_ORDERS);
  }
  const pedidoOriginal = await getOrder(user, idPedido, false, true, true);
  ...
  if (!canCancelOrDelete) {
    if (deleteOrder) throw new DXTException(DXTErrorCode.USER_CANNOT_DELETE_ORDER_IN_CURRENT_STATUS);
  ...
  if (deleteOrder) {
    sendOrderEmail({
      action: EmailOrderAction.delete,
      user,
      order: pedidoOriginal
    });
  } else {
    sendOrderEmail({
      action: EmailOrderAction.cancel,
      user,
      order: pedidoOriginal
    });
  }
...
async function updateRowsPrices(user, ordersById, rows) {
  const orderIds = Object.keys(rows).map((s) => forceToNumber(s));
  const result = {};
  const alicuotas = await alicuotaRepository.getAllPercentages();
  const { listasById } = await getListas();
  const articlesBuffer = /* @__PURE__ */ new Map();
  for (const orderId of orderIds) {
    const sourceRows = rows[orderId];
    const order = ordersById.get(orderId);
    if (order == null) {
      result[orderId] = _nullPrices$1(sourceRows);
      continue;
    }
    const { id_lista, descuento: orderDiscount } = order;
    const lista = listasById.get(id_lista);
    if (lista == null) {
      result[orderId] = _nullPrices$1(sourceRows);
      continue;
    }
    const customer = await clienteRepository.getExtendedByIdOrNull(order.id_cliente);
    if (customer == null) {
      result[orderId] = sourceRows;
      continue;
    }
    let articles = articlesBuffer.get(customer.id);
    if (articles == null) {
      articles = await getArticles(user, customer);
      if (articles != null) articlesBuffer.set(customer.id, articles);
    }
    const { incluye_iva, incluye_ii } = lista;
    const adjustPriceParams = {
      usuarioAplicaSobreIVA: order.cliente_aplica_sobre_iva,
      usuarioAplicaSobreII: order.cliente_aplica_sobre_ii,
      usuarioExentoIVA: order.cliente_exento_iva,
      usuarioExentoII: order.cliente_exento_ii,
      listaIncluyeIVA: incluye_iva,
      listaIncluyeII: incluye_ii
    };
    const newRows = sourceRows.map((row) => {
      const { id_articulo } = row;
      if (id_articulo == null) return row;
      const article = articles.get(id_articulo);
      if (article == null) {
        const renglon2 = removeTaxesFields(row);
        return {
          ...renglon2,
          precio: null,
      ...
      const discountedPrice = invertedTangoDiscount(
        adjustedPrice,
        article?.bonificacion_lista_de_precios,
        orderDiscount
      );
      const renglon = removeTaxesFields(row);
      return {
        ...renglon,
        precio: discountedPrice ?? void 0,
        es_precio_especial: article?.bonificacion_lista_de_precios != null
      };
    });
    result[orderId] = newRows;
  }
  return result;
}
function _nullPrices$1(source) {
...
async function applyTaxesToRows(ordersById, rows) {
  const orderIds = Object.keys(rows).map((s) => forceToNumber(s));
  const result = {};
  const alicuotas = await alicuotaRepository.getAllPercentages();
  for (const orderId of orderIds) {
    ...
    const order = ordersById.get(orderId);
    if (order == null) {
      result[orderId] = _nullPrices(sourceRows);
      continue;
    }
    result[orderId] = applyTaxesToOrder(
      order,
      sourceRows,
      alicuotas
    );
...
async function getAllPedidosRows(user, isDraft) {
  const orderRepository = isDraft ? getDraftRepository(user) : pedidoRepository;
  const rowsRepository = isDraft ? getDraftRowRepository(user) : renglonPedidoRepository;
  const cacheKey = `ALL_${isDraft ? "DRAFT" : "ORDER"}_ROWS_${user.isSeller() ? "S" : "C"}_${user.id.valueOf()}`;
  const cachedData = await rowsRepository.cache.getMetadata(cacheKey);
  if (cachedData != null) return cachedData;
  const rowsByOrder = await rowsRepository.getAllByUser(user);
  const orders = await orderRepository.getAllByUser(user);
  const ordersById = new Map(orders.map((order) => [order.id, order]));
  const result = isDraft ? await updateRowsPrices(user, ordersById, rowsByOrder) : await applyTaxesToRows(ordersById, rowsByOrder);
  await rowsRepository.cache.setMetadata(cacheKey, result);
  return result;
}
const getOrderRowsEndpoint = createApiEndpoint(
  authenticatedRootController,
  void 0,
  async (req) => await getAllPedidosRows(req.auth.user, false)
);
const getDraftRowsEndpoint = createApiEndpoint(
  authenticatedRootController,
  void 0,
  async (req) => await getAllPedidosRows(req.auth.user, true)
);
            ...
            /* @__PURE__ */ jsxs(MenuList, { rootProps: { zIndex: 2e3 }, children: [
              ...
              isAdmin && /* @__PURE__ */ jsxs(Fragment, { children: [
                ...
                /* @__PURE__ */ jsx(
                  MenuItem,
                  {
                    ...
                    children: CONFIGURACION_DEL_SISTEMA
                  }
                )
              ] }),
              /* @__PURE__ */ jsxs(Fragment, { children: [
                ...
                /* @__PURE__ */ jsx(
                  ...
                  {
                    onClick: () => {
                      navigate(URL_VERSION_PATH);
                    },
                    children: `${APP_NAME} ${getProgramVersion()}`
                  }
                )
              ] })
            ] })
            ...
            /* @__PURE__ */ jsx(
              ResponsiveIconButton,
              {
                ...
                sharedProps: {
                  size: "md",
                  fontWeight: "400",
                  colorScheme: isDraft ? "deepOrange" : "orange",
            ...
            !isUpdating && /* @__PURE__ */ jsx(
              ResponsiveIconButton,
              {
                ...
                sharedProps: {
                  size: "md",
                  fontWeight: "400",
                  colorScheme: isDraft ? "green" : "green",
                  variant: isDraft ? "outline" : "solid",
                  isDisabled,
                  onClick: () => {
                    setIsDraft(false);
...
const MessageToUserDialog = ({
  ...
  const exentoIVA = cabecera.usuario_exento_iva;
  const exentoII = cabecera.usuario_exento_ii;
  const allTaxes = !exentoIVA && !exentoII;
  const taxesLabel = allTaxes ? "" : `(${formatTaxesLabel(exentoIVA, exentoII).toLocaleUpperCase()})`;
          ...
          children: [
            ...
            /* @__PURE__ */ jsxs("tbody", { children: [
              renglones.map((product, index) => {
                const { id, codigo, nombre, cantidad, precio, bonificacion_lista_de_precios } = product;
                const precioBonificacion = applyTangoDiscount(
                  precio ?? 0,
                  bonificacion_lista_de_precios ?? bonificacionGlobal
                );
                const productSum = (cantidad ?? 0) * (precioBonificacion ?? 0);
              ...
              /* @__PURE__ */ jsxs("tr", { children: [
                /* @__PURE__ */ jsx("td", { colSpan: 2, children: /* @__PURE__ */ jsxs("strong", { children: [
                  ...
                  bonificacionGlobal > 0 ? /* @__PURE__ */ jsxs(Fragment, { children: [
                    ...
                    /* @__PURE__ */ jsx("span", { children: taxesLabel })
...
const useCustomValidationSchema$1 = (isDraft) => {
  const leyendaError = "La leyenda no debe superar los ${max} caracteres";
  const baseValidationSchema = yup.object({
    ...
    quantities: yup.array().test("at-least-one-quantity", "Ingrese al menos una cantidad", function(value) {
      if (value == null || value.length === 0) return false;
      return value.some((quantity) => quantity != null && quantity > 0);
    }).required("Ingrese al menos una cantidad"),
    leyenda1: yup.string().default("").max(LEYENDA_MAX_LENGTH, leyendaError),
    leyenda2: yup.string().default("").max(LEYENDA_MAX_LENGTH, leyendaError),
    leyenda3: yup.string().default("").max(LEYENDA_MAX_LENGTH, leyendaError),
    leyenda4: yup.string().default("").max(LEYENDA_MAX_LENGTH, leyendaError),
    leyenda5: yup.string().default("").max(LEYENDA_MAX_LENGTH, leyendaError),
    descripcion: yup.string().default("").max(DESCRIPCION_MAX_LENGTH, "La descripción no puede superar los ${max} caracteres"),
    mensaje_interno: yup.string().default("").max(MENSAJE_INTERNO_FIELD_LENGTH, "El mensaje interno no puede superar los ${max} caracteres")
  }).required();
  if (isDraft) {
    ...
    return baseValidationSchema.concat(draftValidationSchema);
  }
                  ...
                  children: [
                    ...
                    calendar.weeks.map((week, weekIdx) => {
                      return week.map((dateObj, index) => {
                        ...
                        if (dateObj == "") return /* @__PURE__ */ jsx(Box, {}, key);
                        const { date } = dateObj;
                        return /* @__PURE__ */ jsx(
                          DayOfMonth,
                          {
...
const OrderTotal = ({ prices, exentoIVA, exentoII }) => {
  const app = useAppResources();
  ...
  const allTaxes = !exentoIVA && !exentoII;
  const taxesLabel = allTaxes ? "" : `${formatTaxesLabel(exentoIVA, exentoII).toLocaleUpperCase()}`;
  const totalLabel = `TOTAL ${bonificacionWatch > 0 ? `-${bonificacionWatch}%` : ""}`;
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx(Heading, { size: "xs", children: /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx("div", { children: totalLabel }),
      /* @__PURE__ */ jsx("div", { children: taxesLabel })
    ] }) }),
...
const OrderHeader$1 = (props) => {
  ...
  const fechaAltaVisible = cabecera.fecha_alta_visible;
  const fechaEntregaVisible = cabecera.fecha_entrega_visible;
  const condicionVentaVisible = !isCustomer && cabecera.condicion_visible;
  const transporteVisible = cabecera.transporte_visible;
  const depositoVisible = cabecera.deposito_visible;
  const direccionVisible = cabecera.direccion_visible;
  const bonificacionVisible = isSeller || cabecera.bonificacion_editable || cabecera.bonificacion > 0;
  const tipoAsientoVisible = !isCustomer && cabecera.asiento_editable && tipoAsientoOptions.length > 0;
  const talonarioVisible = cabecera.talonario_visible;
  const exentoIVA = cabecera.usuario_exento_iva;
  const exentoII = cabecera.usuario_exento_ii;
  ...
  const isFavorite = cabecera.es_favorito;
  const cardBodyProps = {
    borderStyle: isDraft ? "dashed" : void 0,
    borderWidth: isDraft ? "3px" : "0px",
    borderColor: isDraft ? isFavorite ? "orange.500" : "blue.500" : void 0,
    borderRadius: isDraft ? "8px" : void 0,
    p: 0
  };
          ...
          /* @__PURE__ */ jsxs(
            Grid,
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsx(GridItem, { textAlign: { md: "end" }, children: /* @__PURE__ */ jsx(OrderTotal, { prices, exentoIVA, exentoII }) })
              ]
            }
          ),
          /* @__PURE__ */ jsx(Collapse, { in: isOpen, style: { overflow: "inherit" }, children: /* @__PURE__ */ jsxs(
            Grid,
            {
              ...
              children: [
                !isDraft && fechaAltaVisible && /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(
                  ControlledDatePicker,
                  {
                    fieldProps: {
                      name: "fecha_alta",
                      id: "fecha_alta",
                      minDate: getDateOnly(/* @__PURE__ */ new Date()),
                ...
                !isDraft && fechaEntregaVisible && /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(
                  ControlledDatePicker,
                  {
                    fieldProps: {
                      name: "fecha_entrega",
                      id: "fecha_entrega",
                      minDate: getDateOnly(fecha_altaWatch),
                  ...
                  ControlledInput,
                  {
                    fieldProps: {
                      ...
                      onChange: (e) => {
                        ...
                        if (!parsedValue) e.target.value = "0";
                ...
                leyendas,
                /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: showInternalMessage && /* @__PURE__ */ jsx(
                  ControlledTextarea,
                  {
                    fieldProps: {
                      name: "mensaje_interno",
                      id: "mensaje_interno",
                      maxLength: MENSAJE_INTERNO_FIELD_LENGTH,
                      rows: 3,
                      minHeight: "40px",
                      maxHeight: "100px"
                    },
...
const ProductsRow = (props) => {
  const { product, bonificacionGlobal, showPriceColumn } = props;
  const { id, codigo, nombre, descripcion_adicional, precio, bonificacion_lista_de_precios } = product;
  const rowColor = useColorModeValue(
  ...
  const productSum = (productQuantity ?? 0) * (precioBonificacion ?? 0);
  ...
  const discountedPriceClassName = showPriceColumn ? "text-right" : "text-right hidden-on-base";
  const quantityClassName = showPriceColumn ? "hidden-on-base" : "text-center";
  return /* @__PURE__ */ jsxs("tr", { style: { height: "46px", ...productSum && { backgroundColor: rowColor } }, children: [
    /* @__PURE__ */ jsxs("td", { className: "word-break-all", children: [
      codigo,
      " - ",
      formatArticleName(nombre, descripcion_adicional)
    ] }),
    /* @__PURE__ */ jsx("td", { className: "text-right hidden-on-base", children: precio != null && (bonificacion_lista_de_precios == null ? /* @__PURE__ */ jsx(TextPriceNative, { precio, moneda: "$" }) : /* @__PURE__ */ jsx(TextSpecialPrice, {})) }),
    /* @__PURE__ */ jsx("td", { className: discountedPriceClassName, children: precio != null && /* @__PURE__ */ jsx(TextPriceNative, { precio: precioBonificacion, moneda: "$" }) }),
    /* @__PURE__ */ jsx("td", { className: quantityClassName, children: /* @__PURE__ */ jsx("input", { ...inputFieldProps, ...register(`quantities.${id}`) }) }),
...
const ProductsRowWithoutPrice = (props) => {
  ...
  return /* @__PURE__ */ jsxs("tr", { style: { height: "46px" }, children: [
...
const ProductList$1 = memo(function ProductList2({
  articulos,
  showPriceColumn
}) {
  const { control } = useFormContext();
          ...
          children: [
            /* @__PURE__ */ jsx(
              "thead",
              {
                ...
                children: /* @__PURE__ */ jsxs("tr", { children: [
                  ...
                  /* @__PURE__ */ jsx("th", { className: quantityClassName, style: { width: "12%" }, children: "Cantidad" }),
            ...
            /* @__PURE__ */ jsx("tbody", { children: articulos.map((product, index) => {
              if (product.precio == null || product.precio == 0) {
                return /* @__PURE__ */ jsx(ProductsRowWithoutPrice, { product }, product.id);
              }
              return /* @__PURE__ */ jsx(
                ProductsRow,
                {
...
const PRICE_DISPLAY_EVENT = "priceDisplayEvent";
const PriceQuantitySwitch = () => {
  const [displayPrice, setDisplayPrice] = useState(false);
  const handleSwitch = () => {
    setDisplayPrice(!displayPrice);
    eventBus.emit(PRICE_DISPLAY_EVENT, { show: !displayPrice });
  };
  return /* @__PURE__ */ jsx(
    Box,
    {
      ...
      children: /* @__PURE__ */ jsx(
        IconButton,
        {
          ...
          boxShadow: "dark-lg",
          colorScheme: "blue",
          "aria-label": "Mostrar Precio / Cantidad",
          onClick: handleSwitch,
          icon: displayPrice ? /* @__PURE__ */ jsx(CalculatorIcon, {}) : /* @__PURE__ */ jsx(CurrencyUsdIcon, {})
        }
      )
    }
  );
...
const CategoryAccordionButton = (props) => {
  ...
  articulos?.forEach(({ id, precio, bonificacion_lista_de_precios }) => {
    const productQuantity = useWatch({ control, name: `quantities.${id}` });
    if (productQuantity > 0) {
      const precioBonificacion = applyTangoDiscount(
        precio ?? 0,
        bonificacion_lista_de_precios ?? bonificacionWatch
      );
      categorySum = (categorySum != void 0 ? categorySum : 0) + (productQuantity ?? 0) * (precioBonificacion ?? 0);
    }
  });
  return /* @__PURE__ */ jsxs(
    AccordionButton,
...
const ProductsTable = (props) => {
  const { isDraft, isFavorite, unfilteredProducts, filteredProducts, isFiltering } = props;
  ...
  useEffect(() => {
    if (index != null) eventBus.emit("togglePedidoCategory");
  }, [index]);
  useEffect(() => {
    const handleEvent = ({ show }) => {
      setShowPriceColumn(show);
    };
    eventBus.on(PRICE_DISPLAY_EVENT, handleEvent);
    return () => {
      eventBus.removeListener(PRICE_DISPLAY_EVENT, handleEvent);
    };
  }, []);
  useEffect(() => {
    if (index != null && !Array.isArray(index)) {
      setTimeout(() => {
        accordionItemRef.current[index] && accordionItemRef.current[index].scrollIntoView({ behavior: "smooth", block: "start" });
      }, 400);
    }
  }, [index]);
  ...
  var idx = 0;
  const scrollMarginTop = navBarHeight + 5;
  return /* @__PURE__ */ jsx(
    Accordion,
    {
      allowToggle: true,
      ...
      children: _.map(filteredProducts, (articulos, categoria) => {
        ...
        return articulos.length ? /* @__PURE__ */ jsx(
          AccordionItem,
          {
            ...
            ref: (el) => accordionItemRef.current[idx++] = el,
            children: ({ isExpanded }) => /* @__PURE__ */ jsxs(
              CommonCard,
              {
                ...
                children: [
                  ...
                  isExpanded && /* @__PURE__ */ jsx(ProductList$1, { articulos, showPriceColumn })
                ]
              }
            )
          },
...
${ORPHAN_ROWS_FOOTER}`;
  ...
  const yupValidationSchema2 = useCustomValidationSchema$1(isDraft);
  const quantities = [];
  const prices = {};
  _.forOwn(pedidoState.articulos, (articulos) => {
    articulos.forEach((product) => {
      prices[product.id] = {
        ...
        discount: product.bonificacion_lista_de_precios
      };
  ...
  const formMethods = useForm({
    defaultValues: {
      ...
      leyenda1: pedidoState.cabecera.leyenda1,
      leyenda2: pedidoState.cabecera.leyenda2,
      leyenda3: pedidoState.cabecera.leyenda3,
      leyenda4: pedidoState.cabecera.leyenda4,
      leyenda5: pedidoState.cabecera.leyenda5,
      mensaje_interno: pedidoState.cabecera.mensaje_interno ?? void 0
    },
    resolver: yupResolver(yupValidationSchema2)
  });
  const {
  ...
  const quantitiesToRenglones = (quantities2) => {
    ...
    quantities2.forEach((quantity, id) => {
      if (quantity != null && quantity > 0) renglones.push({ id, quantity });
    });
    return renglones;
  };
  const handleCreatePedido = async (dataUnsafe) => {
    const renglones = quantitiesToRenglones(dataUnsafe.quantities.map((q) => forceToNumber(q)));
    const {
  ...
  const handleCreateDraft = async (dataUnsafe) => {
    ...
    result.map({
      success: (state) => {
        toast({
          title: DRAFT_CREATED_TITLE,
          description: DRAFT_CREATED_DESCRIPTION.replace("{{descripcion}}", dataUnsafe.descripcion.toLocaleUpperCase()),
          status: "success"
        });
  ...
  const handleUpdateDraft = async (dataUnsafe) => {
    ...
    const input = {
      ...data,
      id_pedido: pedidoState.cabecera.id_pedido,
      es_favorito: pedidoState.cabecera.es_favorito,
      renglones
    };
    ...
    result.map({
      success: (state) => {
        toast({
          title: DRAFT_UPDATED_TITLE,
          description: DRAFT_UPDATED_DESCRIPTION.replace("{{descripcion}}", dataUnsafe.descripcion.toLocaleUpperCase()),
          status: "success"
        });
        app.navigate(URL_BORRADORES_PATH);
      },
  ...
  useEffect(() => {
    if (Object.keys(errors).length == 1 && errors.quantities) window.scrollTo(0, 0);
  }, [formState]);
  ...
  const isFavorite = pedidoState.cabecera.es_favorito;
  const {
  ...
  return /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsx(FormProvider, { ...formMethods, children: /* @__PURE__ */ jsxs("form", { noValidate: true, children: [
    ...
    /* @__PURE__ */ jsx(OrdersContainer, { children: /* @__PURE__ */ jsxs(Box, { sx: { my: 4, pb: { base: 5, md: void 0 } }, children: [
      /* @__PURE__ */ jsx(FormErrors, { errors }),
      /* @__PURE__ */ jsx(
        OrderHeader$1,
        {
      ...
      /* @__PURE__ */ jsx(PriceQuantitySwitch, {})
    ] }) })
  ] }) }) });
};
function _createUpdateOrderToast(title, saveOrderResult, toast) {
  const { message_to_user, numero_pedido } = saveOrderResult;
  ...
  toast({
    title: formatedTitle,
    description: message_to_user != null ? /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx(Divider, { mt: 2, mb: 1 }),
      /* @__PURE__ */ jsx("b", { children: message_to_user }),
      /* @__PURE__ */ jsx(Divider, { mt: 1, mb: 2 })
    ] }) : void 0,
    status: "success"
  });
}
...
const _OrderEditPage = (props) => {
  ...
  if (!idPedidoIsValid) return /* @__PURE__ */ jsx(OrderInvalidParameterError, { isDraft });
...
const ProductListLoading = (props) => {
  const { options } = props;
  return /* @__PURE__ */ jsx(
    Box,
    {
      ...
      children: /* @__PURE__ */ jsxs(
        Grid,
        {
          ...
          children: [
            ...
            /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(Text, { fontSize: "sm", children: options.description }) }),
            /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(FormInputSkeleton, {}) }),
            /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(FormInputSkeleton, {}) })
          ]
        }
...
const _productListValidationSchema = yup.object({
  list: yup.string().default("").test("avoid-ARTICLE_GROUP_NO_NAME", `Evite utilizar ${ARTICLE_GROUP_NO_NAME}`, function(value) {
    const regex = /(^|[^a-zA-Z0-9])_([^a-zA-Z0-9]|$)/;
    return !regex.test(value);
  })
}).required();
const productEditListValidationSchema = _productListValidationSchema.concat(
  yup.object({
    list_enabled: yup.boolean().required(),
    group_separator: yup.number().typeError("Ingrese un separador de grupo").test(
      "is-group-separator",
      "Ingrese un separador de grupo válido",
      (v) => yupValidation(() => numberToArticleGroupSeparator(v))
    ).required()
  }).required()
);
const productPrintListValidationSchema = _productListValidationSchema;
const ProductPrintListReady = (props) => {
  const { stateData, options } = props;
  const app = useAppResources();
  const toast = useToast();
  const {
    handleSubmit,
    ...
    resolver: yupResolver(productPrintListValidationSchema)
  });
  const disableForm = isSubmitSuccessful || isSubmitting;
  const onSubmit = async (dataUnsafe) => {
    const input = { lista: dataUnsafe.list.split("\n") };
    const result = await options.apiPost(input, app);
    result.map({
      success: (_2) => {
        toast({
          title: LIST_UPDATED,
...
const ProductEditListReady = (props) => {
  const { stateData, options } = props;
  const app = useAppResources();
  const toast = useToast();
  const {
    ...
    defaultValues: {
      list_enabled: stateData.lista_activa,
      group_separator: stateData.separador_grupo,
      list: stateData.lista.join("\r\n") || ""
    },
    resolver: yupResolver(productEditListValidationSchema)
  });
  const handleSetListEnabled = (newValue) => {
    setValue("list_enabled", newValue == "1" ? true : false);
  };
  const disableForm = isSubmitSuccessful || isSubmitting;
  const watchListEnabled = watch("list_enabled");
  const onSubmit = async (dataUnsafe) => {
    const input = {
      lista: dataUnsafe.list.split("\n"),
      lista_activa: dataUnsafe.list_enabled,
      separador_grupo: dataUnsafe.group_separator
    };
    const result = await options.apiPost(input, app);
    result.map({
      success: (_2) => {
  ...
  return /* @__PURE__ */ jsx("form", { noValidate: true, onSubmit: handleSubmit(onSubmit), children: /* @__PURE__ */ jsxs(Box, { children: [
    /* @__PURE__ */ jsx(FormErrors, { errors }),
    /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsx(Grid, { gap: 4, children: /* @__PURE__ */ jsxs(RadioGroup, { onChange: handleSetListEnabled, value: watchListEnabled ? "1" : "0", children: [
      /* @__PURE__ */ jsx(Radio, { value: "0", mb: 2, children: "Lista agrupada por código" }),
      /* @__PURE__ */ jsx(
        ControlledSelect,
        {
          fieldProps: {
            ...
            options: [
              ...[
                { label: "Ninguno", value: ArticleGroupSeparator.none },
                { label: "Espacio", value: ArticleGroupSeparator.space },
                { label: "Guión", value: ArticleGroupSeparator.dash },
                { label: "Guión bajo", value: ArticleGroupSeparator.underscore },
                { label: "Letras y números", value: ArticleGroupSeparator.lettersAndNumbers }
              ]
            ],
            selectedOptionStyle: "check"
          },
          formControlProps: {
            isDisabled: disableForm || watchListEnabled,
            width: "auto",
            maxWidth: 200,
            mb: 4
          },
          formControlInnerProps: {
            label: "Separador de grupo"
          },
          control
        }
      ),
      /* @__PURE__ */ jsx(Radio, { value: "1", mb: 2, children: "Lista personalizada" }),
      /* @__PURE__ */ jsxs(
        Grid,
        {
          templateColumns: { base: "1fr", md: "repeat(2,1fr)" },
...
var ProductListType = /* @__PURE__ */ ((ProductListType2) => {
  ProductListType2[ProductListType2["edit"] = 0] = "edit";
  ProductListType2[ProductListType2["print"] = 1] = "print";
  return ProductListType2;
})(ProductListType || {});
const productEditListOptions = {
  apiGetAll: useGetDXTArticuloEditList,
  apiPost: dxtArticuloEditListUpdateRequest,
...
const productPrintListOptions = {
  apiGetAll: useGetDXTArticuloPrintList,
  apiPost: dxtArticuloPrintListUpdateRequest,
...
const ProductList = (props) => {
  const navigate = useNavigate();
  const { type } = props;
  const options = type == 0 ? productEditListOptions : productPrintListOptions;
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx(
      SettingsFormHeading,
      {
    ...
    /* @__PURE__ */ jsx(_ProductList, { type, options })
  ] });
};
const _ProductList = ({ options, type }) => {
  const { state, retry } = options.apiGetAll();
  return state.map({
    loading: (_2) => /* @__PURE__ */ jsx(ProductListLoading, { options }),
    error: ({ error }) => /* @__PURE__ */ jsx(
      ApiErrors,
      {
        error,
    ...
    success: (state2) => type == 0 ? /* @__PURE__ */ jsx(ProductEditListReady, { stateData: state2.data, options }) : /* @__PURE__ */ jsx(ProductPrintListReady, { stateData: state2.data, options })
  });
};
function Lists$2() {
  return /* @__PURE__ */ jsx(ProductList, { type: ProductListType.print });
}
...
const TextWordBreak = ({
  ...
  return /* @__PURE__ */ jsx(
    Text,
    {
      ...
      sx: {
        whiteSpace: "normal",
        wordBreak: breakType ?? "break-word",
        ...props?.sx
      },
      children
    }
...
function Lists() {
  return /* @__PURE__ */ jsx(ProductList, { type: ProductListType.edit });
}
...
const setArticuloPrintListInputSchema = {
  lista: (v) => new VOArticuloListRecords(v)
};
const validateArticuloPrintListInput = (input) => validateInput(setArticuloPrintListInputSchema, input);
const setArticuloEditListInputSchema = {
  lista: (v) => new VOArticuloListRecords(v),
  lista_activa: (v) => new VOBoolean(v),
  separador_grupo: (v) => numberToArticleGroupSeparator(v)
};
const validateArticuloEditListInput = (input) => validateInput(setArticuloEditListInputSchema, input);
...
const dxtArticuloGetEditListEndpoint = createApiEndpoint(
  ...
  void 0,
  async (req) => {
    const { article_edit_list_enabled: lista_activa, article_edit_list_group_separator: separador_grupo } = await settingsProvider.getOrderSettings();
    const lista = await dxtArticuloEditListRepository.getList();
    return {
      lista,
      lista_activa,
      separador_grupo
...
const dxtArticuloSetPrintListEndpoint = createApiEndpoint(
  ...
  {
    validation: {
      body: validateArticuloPrintListInput
    }
  },
  async (req) => {
    await dxtArticuloPrintListRepository.setList(req.validated.body.lista);
    return { ok: true };
  }
);
const dxtArticuloSetEditListEndpoint = createApiEndpoint(
  ...
  {
    validation: {
      body: validateArticuloEditListInput
    }
  },
  async (req) => {
    const {
    ...
    const orderSettings = await settingsProvider.getOrderSettings();
    const newOrderSettings = new StoredOrderSettingsDTO({
      ...orderSettings,
      article_edit_list_enabled: lista_activa,
      article_edit_list_group_separator: separador_grupo
    });
    await settingsProvider.setOrderSettings(newOrderSettings);
    await dxtArticuloEditListRepository.setList(lista);
    return { ok: true };
  }
);
  ...
  async (req) => {
    ...
    for (const [key, companyUnsafe] of companies.entries()) {
      if (company === companyUnsafe.db_name || company.toLowerCase() === companyUnsafe.name.toLowerCase()) {
        ...
        await settingsProvider.setDBSettings(newDBSettingsDTO);
        emitBusEvent(
          new DBSettingsChangedEvent({
            passiveDBChanges: false,
            newDictionary: false,
...
const loader$z = unimplementedApiResponse;
...
const loader$y = unimplementedApiResponse;
...
const loader$t = async (o) => await dxtUsuarioGetAuxiliaresEndpoint.get(o.request, o.params);
...
const OrdersNav = ({
  ...
  const handlePrint = () => {
    if (Array.isArray(selectedPedidos))
      window.open(`${URL_PEDIDOS_PRINT_PATH}?orderId=${selectedPedidos.join("&orderId=")}`, "OrderPrintWindow", "width=800,height=600");
  };
...
function calculateRealOrderStatus(pedido, stateRenglones) {
  if (!stateRenglones.isSuccess()) return pedido.estado;
...
const PedidoMenu = ({
  ...
  if (isDraft && !isFavorite) {
    ...
    if (menuItems.length > 0) menuItems.push(/* @__PURE__ */ jsx(MenuDivider, {}, menuItems.length));
    menuItems.push(
      /* @__PURE__ */ jsx(
        MenuItem,
        {
  ...
  if (isDraft || isUserAllowedToCancelOrDeleteOrder(user, pedido.estado)) {
    if (menuItems.length > 0) menuItems.push(/* @__PURE__ */ jsx(MenuDivider, {}, menuItems.length));
    menuItems.push(
      /* @__PURE__ */ jsx(
        MenuItem,
  ...
  const disableMenu = menuItems.length == 0;
  return /* @__PURE__ */ jsxs(Menu, { isLazy: true, id: "menu", children: [
    ...
    /* @__PURE__ */ jsx(Portal, { children: /* @__PURE__ */ jsx(MenuList, { rootProps: { zIndex: 2e3 }, children: menuItems }) })
  ] });
};
...
const RenglonesPedido = ({
  ...
  const showDiscount = descuento > 0 || !esCliente;
  return /* @__PURE__ */ jsxs(
    "table",
    {
      width: "100%",
      ...
      children: [
        /* @__PURE__ */ jsx(
          "thead",
          {
            ...
            children: /* @__PURE__ */ jsxs("tr", { children: [
              /* @__PURE__ */ jsx("th", { className: "text-left", style: { width: showDiscount ? "60%" : "70%" }, children: "Artículo" }),
              /* @__PURE__ */ jsx("th", { className: "text-right hidden-on-base", style: { width: "10%" }, children: "Precio" }),
              showDiscount && /* @__PURE__ */ jsx("th", { className: "text-right hidden-on-mobile", style: { width: "10%" }, children: "Bonificado" }),
              /* @__PURE__ */ jsx("th", { className: "text-center", style: { width: "10%" }, children: "Cantidad" }),
              /* @__PURE__ */ jsx("th", { className: "text-right", style: { width: "10%" }, children: "Subtotal" })
            ] })
          }
        ),
        /* @__PURE__ */ jsx("tbody", { children: renglones.map(
          ({
            ...
            const precioBonificado = applyTangoDiscount(precio, descuento);
            const subtotalBonificado = applyTangoDiscount(subtotal, descuento);
            return /* @__PURE__ */ jsxs("tr", { style: { color: precio == null ? "red" : void 0 }, children: [
              /* @__PURE__ */ jsx("td", { children: /* @__PURE__ */ jsx(TextWordBreak, { children: fullArticleDescription(
                nombre_articulo,
                codigo_articulo,
                descripcion_adicional
              ) }) }),
              /* @__PURE__ */ jsx("td", { className: "text-right hidden-on-base", children: es_precio_especial && precio != null ? "Especial" : /* @__PURE__ */ jsx(TextPriceNative, { precio, moneda: "$" }) }),
              showDiscount && /* @__PURE__ */ jsx("td", { className: "text-right hidden-on-mobile", children: /* @__PURE__ */ jsx(TextPriceNative, { precio: precioBonificado, moneda: "$" }) }),
              /* @__PURE__ */ jsx("td", { children: /* @__PURE__ */ jsx(Text, { textAlign: "center", children: cantidad }) }),
              /* @__PURE__ */ jsx("td", { className: "text-right", children: /* @__PURE__ */ jsx(TextPriceNative, { precio: subtotalBonificado, moneda: "$" }) })
            ] }, `details-${numero_pedido}-${index}`);
          }
        ) })
      ]
    }
...
const Renglones = ({ wasOpen, pedido, stateRenglones, esCliente }) => {
  return /* @__PURE__ */ jsx(Box, { sx: { mt: 6 }, children: wasOpen && stateRenglones.map({
    ...
    success: (state) => {
      ...
      if (!Array.isArray(renglones)) return /* @__PURE__ */ jsx(RenglonesEmpty, {});
      return /* @__PURE__ */ jsx(
        RenglonesPedido,
        {
          pedido,
...
const FlexHeaderGrid = (props) => {
  const { children } = props;
  return /* @__PURE__ */ jsx(Flex, { direction: { base: "row", lg: "column" }, gap: 4, children });
};
const FlexHeaderGridItem = (props) => {
  const { children } = props;
  return /* @__PURE__ */ jsx(Flex, { direction: "column", flex: 1, children });
};
function TotalGridItem({ total, descuento, taxesLabel }) {
  const totalLabel = `TOTAL ${descuento > 0 ? `-${descuento}%` : ""}`;
  return /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
    /* @__PURE__ */ jsxs(
      Heading,
      {
        size: "sm",
        sx: {
          textTransform: "uppercase",
          // color: estado === EstadoPedido.APROBADO && 'blue.500',
          whiteSpace: "normal"
        },
        children: [
          /* @__PURE__ */ jsx("div", { children: totalLabel }),
...
const OrderHeader = (props) => {
  ...
  const {
    ...
    leyenda4,
    leyenda5,
    cliente_exento_iva: exentoIVA,
    cliente_exento_ii: exentoII
  } = pedido;
  const isSeller = app.authState.isSeller();
  const isAdmin = app.authState.isAdmin();
  const captions = orderCaptionsToArray(leyenda1, leyenda2, leyenda3, leyenda4, leyenda5);
  const showCaptions = captions.length > 0;
  const showInternalMessage = mensaje_interno != null && mensaje_interno.length > 0;
  const allTaxes = !exentoIVA && !exentoII;
  const taxesLabel = allTaxes ? "" : `${formatTaxesLabel(exentoIVA, exentoII)}`;
  return /* @__PURE__ */ jsxs(
    Grid,
    {
      templateColumns: {
      ...
      children: [
        /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsxs(FlexHeaderGrid, { children: [
          /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
          ...
          /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
            ...
            /* @__PURE__ */ jsx(Box, { children: /* @__PURE__ */ jsx(BadgePedidosEstado, { estado }) })
          ] })
        ] }) }),
        /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsxs(FlexHeaderGrid, { children: [
          /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
            /* @__PURE__ */ jsx(Heading, { size: "sm", children: "Ingreso" }),
            /* @__PURE__ */ jsx(Text, { children: datetimeToLocale(fecha_hora_ingreso) })
          ] }),
          fecha_entrega != null && /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
            /* @__PURE__ */ jsx(Heading, { size: "sm", children: "Entrega" }),
            /* @__PURE__ */ jsx(Text, { children: dateToLocale(fecha_entrega) })
          ] })
        ] }) }),
        (isAdmin || isSeller) && /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsxs(FlexHeaderGrid, { children: [
          /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
          ...
          !isSeller && /* @__PURE__ */ jsxs(FlexHeaderGridItem, { children: [
            /* @__PURE__ */ jsxs(Heading, { size: "sm", children: [
              "Vendedor ",
              codigo_vendedor
            ] }),
        ...
        /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsxs(FlexHeaderGrid, { children: [
          ...
          /* @__PURE__ */ jsx(TotalGridItem, { total, descuento, taxesLabel })
        ] }) }),
        showCaptions && /* @__PURE__ */ jsx(GridItem, { colSpan: { lg: 4 }, children: /* @__PURE__ */ jsx(Alert, { status: "warning", sx: { flexDirection: "column", alignItems: "flex-start", bgColor: "yellow.300" }, children: captions.map((c, i) => /* @__PURE__ */ jsx(Box, { sx: { whiteSpace: "normal", marginBottom: 1 }, children: c.v }, i)) }) }),
        showInternalMessage && /* @__PURE__ */ jsx(GridItem, { colSpan: { lg: 4 }, children: /* @__PURE__ */ jsx(Alert, { status: "info", sx: { flexDirection: "column", alignItems: "flex-start", whiteSpace: "normal" }, children: nlToReactBr(mensaje_interno) }) })
      ]
    }
  );
};
const DraftHeader = (props) => {
  ...
  return /* @__PURE__ */ jsxs(
    Grid,
    {
      ...
      children: [
        /* @__PURE__ */ jsx(GridItem, { colSpan: { lg: 3, md: 2 }, children: /* @__PURE__ */ jsx(Heading, { size: "md", textTransform: "uppercase", children: descripcion }) }),
        ...
        /* @__PURE__ */ jsx(TotalGridItem, { total, descuento, taxesLabel }),
        showCaptions && /* @__PURE__ */ jsx(GridItem, { colSpan: { lg: 3, md: 2 }, children: /* @__PURE__ */ jsx(Alert, { status: "warning", sx: { flexDirection: "column", alignItems: "flex-start", bgColor: "yellow.300" }, children: captions.map((c, i) => /* @__PURE__ */ jsx(Box, { sx: { whiteSpace: "normal", marginBottom: 1 }, children: c.v }, i)) }) }),
        showInternalMessage && /* @__PURE__ */ jsx(GridItem, { colSpan: { lg: 3, md: 2 }, children: /* @__PURE__ */ jsx(Alert, { status: "info", sx: { flexDirection: "column", alignItems: "flex-start", whiteSpace: "normal" }, children: nlToReactBr(mensaje_interno) }) })
      ]
    }
  );
};
function calculateRealOrderTotal(pedido, stateRenglones) {
  if (!stateRenglones.isSuccess()) return null;
...
const Pedido = ({
  ...
  const renglonesComponent = useMemo(
    () => /* @__PURE__ */ jsx(
      Renglones,
      {
        ...
        pedido,
        stateRenglones,
        esCliente: app.authState.isCustomer()
      }
    ),
  ...
  const handleToggleDetails = () => {
    ...
    if (nowIsOpen) wasOpen.current = true;
    setIsOpen(nowIsOpen);
  ...
  useEffect(() => {
    const handleEvent = ({ id: id2 }) => {
      if (pedido.id !== id2) setIsOpen(false);
    };
...
function PedidoList(props) {
  ...
  const handleMarkAsFavorite = async (draftId, isFavorite) => {
    if (isFavorite) {
      ...
      if (result.isSuccess()) await reloadData();
      return;
    }
    const dialogResult = await confirmationDialog({
...
function _draftCmp(a, b) {
  ...
  if (favDif != 0) return favDif;
  const aIngreso = new Date(a.fecha_hora_ingreso).getTime();
  const bIngreso = new Date(b.fecha_hora_ingreso).getTime();
  return bIngreso - aIngreso;
}
function _orderCmp$1(a, b) {
  const estadoDif = a.estado - b.estado;
  if (estadoDif != 0) return estadoDif;
...
const OrdersPrintReady = (props) => {
  const { orders, fixedPageWidth } = props;
  const [scale, setScale] = useState(1);
  const [pageBreaks, setPageBreaks] = useState(false);
  const handlePageBreaks = () => {
    setPageBreaks(!pageBreaks);
  };
  return /* @__PURE__ */ jsxs(Box, { sx: { backgroundColor: "white", minHeight: "100vh", transform: `scale(${scale})` }, children: [
    ...
    orders.map((order, index) => {
      ...
      const showCaptions = captions.length > 0;
      const showInternalMessage = mensaje_interno != null;
      const taxesLabel = formatTaxesLabel(exentoIVA, exentoII);
      ...
      return /* @__PURE__ */ jsxs(React.Fragment, { children: [
        ...
        /* @__PURE__ */ jsx(TableContainer, { sx: { p: 2, overflowX: "hidden" }, children: /* @__PURE__ */ jsxs(Table, { variant: "basicPrintable", children: [
          /* @__PURE__ */ jsxs(Thead, { children: [
            /* @__PURE__ */ jsx(Tr, { children: /* @__PURE__ */ jsx(Th, { colSpan: 5, children: /* @__PURE__ */ jsxs(Grid, { className: "order_print", templateColumns: "min-content auto max-content", gap: 4, children: [
              /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsxs("table", { width: "auto", children: [
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Número" }),
                  /* @__PURE__ */ jsx("td", { className: "v", children: /* @__PURE__ */ jsx("strong", { children: numero_pedido }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Estado" }),
                  /* @__PURE__ */ jsx("td", { className: "v", children: /* @__PURE__ */ jsx("strong", { style: { textTransform: "uppercase" }, children: getEstadoPedidoText(estado) }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Ingreso" }),
                  /* @__PURE__ */ jsx("td", { className: "v", children: /* @__PURE__ */ jsx("strong", { children: dateToLocale(fecha_hora_ingreso) }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", {}),
                  /* @__PURE__ */ jsx("td", { className: "v", children: /* @__PURE__ */ jsx("strong", { children: timeToLocale(fecha_hora_ingreso) }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: fecha_entrega != null && /* @__PURE__ */ jsx(Fragment, { children: "Entrega" }) }),
                  /* @__PURE__ */ jsx("td", { className: "v", children: fecha_entrega != null && /* @__PURE__ */ jsx("strong", { children: dateToLocale(fecha_entrega) }) })
                ] })
              ] }) }),
              /* @__PURE__ */ jsx(GridItem, { display: "flex", justifyContent: "center", alignItems: "flex-start", minWidth: "250px", children: /* @__PURE__ */ jsxs("table", { children: [
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Cliente" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: formatClienteVendedor(codigo_cliente, nombre_cliente) }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Direccion" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: direccion_cliente ?? "-" }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Localidad" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: localidad_cliente ?? "-" }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Teléfono" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: telefono_cliente ?? "-" }) })
                ] })
              ] }) }),
              /* @__PURE__ */ jsx(GridItem, { width: "fit-content", maxWidth: "250px", children: /* @__PURE__ */ jsxs("table", { children: [
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Vendedor" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: formatClienteVendedor(codigo_vendedor, nombre_vendedor) }) })
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Deposito" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: formatAuxiliares(codigo_deposito, nombre_deposito) }) }),
                  /* @__PURE__ */ jsx("td", {})
                ] }),
                /* @__PURE__ */ jsxs("tr", { children: [
                  /* @__PURE__ */ jsx("td", { className: "l", children: "Transporte" }),
                  /* @__PURE__ */ jsx("td", { className: "v word-break-all", children: /* @__PURE__ */ jsx("strong", { children: formatAuxiliares(codigo_transporte, nombre_transporte) }) })
                ] })
              ] }) })
            ] }) }) }),
            /* @__PURE__ */ jsxs(Tr, { children: [
              ...
              /* @__PURE__ */ jsx(Th, { textAlign: "right", width: "100px", children: "Bonificado" }),
              /* @__PURE__ */ jsx(Th, { textAlign: "center", width: "80px", children: "Cantidad" }),
              /* @__PURE__ */ jsx(Th, { textAlign: "right", width: "100px", children: "Subtotal" })
            ] })
          ] }),
          /* @__PURE__ */ jsxs(Tbody, { className: "rows", children: [
            order.renglones != null && order.renglones.map((renglon, index2) => {
              ...
              return /* @__PURE__ */ jsxs(Tr, { children: [
                ...
                /* @__PURE__ */ jsx(Td, { children: fullArticleDescription(nombre_articulo, codigo_articulo, descripcion_adicional) }),
                /* @__PURE__ */ jsx(Td, { textAlign: "right", children: /* @__PURE__ */ jsx(TextPriceNative, { moneda: "$", precio: precioBonificado }) }),
                ...
                /* @__PURE__ */ jsx(Td, { textAlign: "right", children: /* @__PURE__ */ jsx(TextPriceNative, { moneda: "$", precio: subtotalBonificado }) })
              ] }, `renglon-${id}_${id_articulo}`);
            }),
            /* @__PURE__ */ jsxs(Tr, { children: [
              /* @__PURE__ */ jsxs(Td, { colSpan: 4, valign: "top", fontWeight: "bold", children: [
                /* @__PURE__ */ jsx("p", { children: `TOTAL ${descuento > 0 ? `CON BONIFICACIÓN DEL ${descuento}% ` : ""}` }),
                /* @__PURE__ */ jsx("p", { children: taxesLabel.toLocaleUpperCase() })
              ] }),
              /* @__PURE__ */ jsx(Td, { textAlign: "right", valign: "bottom", fontWeight: "bold", children: /* @__PURE__ */ jsx(TextPriceNative, { moneda: "$", precio: total }) })
            ] })
          ] }),
          showCaptions && /* @__PURE__ */ jsxs(Tbody, { className: "captions", children: [
            /* @__PURE__ */ jsx(Tr, { children: /* @__PURE__ */ jsxs(Td, { colSpan: 5, children: [
              /* @__PURE__ */ jsx("br", {}),
              /* @__PURE__ */ jsx("b", { children: "Leyendas" })
            ] }) }),
            captions.map((c, i) => /* @__PURE__ */ jsx(Tr, { children: /* @__PURE__ */ jsx(Td, { colSpan: 5, children: c.v }) }))
          ] }),
          showInternalMessage && /* @__PURE__ */ jsxs(Tbody, { className: "captions", children: [
            /* @__PURE__ */ jsx(Tr, { children: /* @__PURE__ */ jsxs(Td, { colSpan: 5, children: [
              /* @__PURE__ */ jsx("br", {}),
              /* @__PURE__ */ jsx("b", { children: "Mensaje interno" })
            ] }) }),
            /* @__PURE__ */ jsx(Tr, { children: /* @__PURE__ */ jsx(Td, { colSpan: 5, height: 50, valign: "top", children: mensaje_interno }) })
          ] })
        ] }) }),
        /* @__PURE__ */ jsx("div", { style: { borderTop: "2px solid #000" } })
      ] }, `pedido-${id}`);
    })
  ] });
};
const PrintOrderPage = (props) => {
  ...
  if (orders != null && Array.isArray(orders)) {
    ...
    orders.forEach((order) => {
      ...
      if (idValidated != null) safeOrders.push(idValidated);
    });
    if (safeOrders.length > 0) return /* @__PURE__ */ jsx(_PrintOrderPage, { orders: safeOrders, fixedPageWidth });
  }
  return /* @__PURE__ */ jsx(
    CommonErrors,
    {
...
const _PrintOrderPage = (props) => {
  ...
  return state.map({
    ...
    success: (ordersState) => {
      ...
      return /* @__PURE__ */ jsx(OrdersPrintReady, { orders: ordersState.data, fixedPageWidth });
    }
  });
};
...
const FIXED_PRINT_PAGE_WIDTH = 1920;
const meta = () => [
  { charset: "utf-8" },
  {
    name: "viewport",
    content: `width=${FIXED_PRINT_PAGE_WIDTH}, initial-scale=1.0`
  }
];
function OrdersPrint() {
  const [searchParams] = useSearchParams();
  const ordersIds = searchParams.getAll("orderId");
  return /* @__PURE__ */ jsx(PrintOrderPage, { orders: ordersIds, fixedPageWidth: FIXED_PRINT_PAGE_WIDTH });
}
  ...
  return /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsxs(VStack, { spacing: 4, width: "full", children: [
    ...
    /* @__PURE__ */ jsx(Box, { width: "full", children: /* @__PURE__ */ jsx(
      SettingsListItem,
      {
        ...
        actionButtonState: SettingsActionButtonState.enabled,
        actionButtonOnClick: () => {
          navigate(URL_SETTINGS_ORDER_PATH);
        }
      }
    ) })
  ] }) });
...
async function getDownloadableFiles() {
  const { downloads_path } = await settingsProvider.getProgramConfig();
  if (downloads_path == "") return [];
  let result = [];
  try {
    const items = await readdir(downloads_path, { withFileTypes: true });
    for (const item of items) {
      if (item.isFile()) result.push(item.name);
    }
  } catch (err) {
  }
  return result;
...
async function downloadFile(filename) {
  ...
  if (!downloadableFiles.includes(filename)) throw new DXTException(DXTErrorCode.NOT_FOUND);
  const { downloads_path } = await settingsProvider.getProgramConfig();
  const filePath = path__default.join(downloads_path, filename);
  try {
    const stats = await stat(filePath);
    return {
      stream: fs__default.createReadStream(filePath),
      fileSize: stats.size
    };
  } catch (e) {
...
const API_SETTINGS_ORDER = apiEndpoint("/settings/order", "GET");
const API_SETTINGS_ORDER_SET = apiEndpoint("/settings/order", "POST");
const SettingsOrderLoading = () => /* @__PURE__ */ jsx(
  Box,
  {
    width: "full",
    sx: {
...
const settingsOrderUpdateRequest = async (input, app) => {
  return await dxtApiRequest(
    {
      ...API_SETTINGS_ORDER_SET,
      data: input,
      silent: true
    },
    app
...
const yupValidationSchema$2 = yup.object({
  show_shipping_date_to_customers: yup.boolean().required(),
  order_create_update_message: yup.string().required("Ingrese un mensaje de aclaración para correos de pedido")
}).required();
const SettingsOrderReady = (props) => {
  const { stateData } = props;
  const app = useAppResources();
  const toast = useToast();
  const {
  ...
  const onSubmit = async (dataUnsafe) => {
    ...
    const result = await settingsOrderUpdateRequest(input, app);
    result.map({
      success: (_2) => {
        toast({
        ...
        children: [
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(
            ControlledCheckbox,
            {
              control,
              fieldProps: {
                id: "show_shipping_date_to_customers",
                name: "show_shipping_date_to_customers"
              },
              formControlProps: {
                isDisabled: disableForm
              },
              formControlInnerProps: {
                text: "Los clientes pueden ver la fecha de entrega"
              }
            }
          ) }),
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(
            ...
            {
              fieldProps: {
                name: "order_create_update_message",
                id: "order_create_update_message",
                isDisabled: disableForm
              },
              formControlProps: {
                isDisabled: disableForm
              ...
              formControlInnerProps: {
                label: "Mensaje al crear o modificar pedido"
              },
              control
            }
          ) })
...
const SettingsOrder = () => {
  ...
  return state.map({
    loading: (_2) => /* @__PURE__ */ jsx(SettingsOrderLoading, {}),
    error: ({ error }) => /* @__PURE__ */ jsx(
      ApiErrors,
      {
        error,
    ...
    success: (state2) => /* @__PURE__ */ jsx(SettingsOrderReady, { stateData: state2.data })
  });
};
function Misc$1() {
  const navigate = useNavigate();
...
const DownloadsReady = ({ files }) => {
  ...
  files = [];
  return /* @__PURE__ */ jsx(Box, { children: /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsx(LightMode, { children: /* @__PURE__ */ jsx(VStack, { spacing: 4, minHeight: 300, justifyContent: "center", children: files.length > 0 ? files.map((filename) => /* @__PURE__ */ jsx(DownloadItem, { filename })) : /* @__PURE__ */ jsx(Fragment, { children: NO_DOWNLOADS_AVAILABLE }) }) }) }) });
};
...
const loader$q = unimplementedApiResponse;
...
const yupValidationSchema = yup.object({
  smtp_enabled: yup.boolean().required(),
  ...
  smtp_from_email: yup.string().required("Ingrese el correo electrónico de salida").email("Ingrese un correo electrónico válido"),
  ...
  order_warning_message: yup.string()
}).required();
const SettingsMiscReady = (props) => {
  ...
  const watchSmtpEnabled = watch("smtp_enabled");
  const disableForm = isSubmitSuccessful || isSubmitting;
  const disableSMTPForm = disableForm || !watchSmtpEnabled;
  const onSubmit = async (dataUnsafe) => {
    const input = dataUnsafe;
  ...
  return /* @__PURE__ */ jsx("form", { noValidate: true, onSubmit: handleSubmit(onSubmit), children: /* @__PURE__ */ jsxs(Box, { children: [
        ...
        children: [
          ...
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(
            ControlledTextarea,
            {
              ...
              formControlInnerProps: {
                label: "Contenido del mensaje de advertencia"
              },
              control
            }
          ) }),
          ...
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(
            ControlledTextarea,
            {
              ...
              formControlInnerProps: {
                label: "Contenido del mensaje de inhabilitación"
              },
              control
            }
          ) })
    ...
    /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsxs(
      Grid,
      {
        ...
        gap: 4,
        children: [
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(Heading, { size: "sm", textTransform: "uppercase", children: "Administrador" }) }),
          /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(
            ControlledInput,
            {
              fieldProps: {
    ...
    /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsxs(
      Grid,
      {
        ...
        gap: 4,
        children: [
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(Heading, { size: "sm", textTransform: "uppercase", children: "Sesiones" }) }),
          /* @__PURE__ */ jsx(GridItem, { children: /* @__PURE__ */ jsx(
            ControlledInput,
            {
              fieldProps: {
              ...
              formControlInnerProps: {
                label: "Tiempo de espera para logout (en días)"
              },
              control
            }
          ) })
    ...
    /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsxs(
      Grid,
      {
        ...
        gap: 4,
        children: [
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(Heading, { size: "sm", textTransform: "uppercase", children: "Envío de correo SMTP" }) }),
          /* @__PURE__ */ jsx(GridItem, { colSpan: { md: 2 }, children: /* @__PURE__ */ jsx(
            ControlledCheckbox,
            {
              control,
...
const loader$p = unimplementedApiResponse;
...
const loader$m = async () => {
  return json({
    changelog: changeLogFromNode
  });
};
function VersionInfo() {
  const data = useLoaderData();
  const versionTitleColor = useColorModeValue("blue.800", "blue.200");
  const sectionTitleColor = useColorModeValue("green.800", "green.200");
  const renderer = useMemo(() => ChakraUIRenderer({
    h2: (props) => /* @__PURE__ */ jsx(Heading, { as: "h3", size: "md", mt: 12, mb: 4, color: versionTitleColor, children: props.children }),
    h3: (props) => /* @__PURE__ */ jsx(Heading, { as: "h4", size: "sm", mt: 6, mb: 2, color: sectionTitleColor, children: props.children })
  }), [versionTitleColor]);
  return /* @__PURE__ */ jsxs(Box, { pb: 1, children: [
    /* @__PURE__ */ jsx(Navbar, {}),
    /* @__PURE__ */ jsx(Container, { maxW: "2xl", sx: { my: 4 }, children: /* @__PURE__ */ jsx(CommonCard, { children: /* @__PURE__ */ jsx(
      ReactMarkdown,
      {
        options: {
          forceBlock: false,
...
const getOrderSettingsEndpoint = createApiEndpoint(
  adminRootController,
  void 0,
  async (req) => await settingsProvider.getOrderSettings(true)
);
...
const miscSettingsInputSchema = {
  smtp_enabled: (v) => new VOBoolean(v),
  ...
  smtp_from_email: (v) => new VOEmailAddress(v).valueOf(),
...
const orderSettingsInputSchema = {
  show_shipping_date_to_customers: (v) => new VOBoolean(v),
  order_create_update_message: (v) => new VOString(v.trim())
};
const validateOrderSettingsInput = (input) => validateInput(orderSettingsInputSchema, input);
const updateOrderSettingsEndpoint = createApiEndpoint(
  adminRootController,
  {
    validation: {
      body: validateOrderSettingsInput
    }
  },
  async (req) => {
    const { body } = req.validated;
    const newSettingsDTO = new StoredOrderSettingsDTO(body);
    await settingsProvider.setOrderSettings(newSettingsDTO);
    return body;
  }
);
async function loader$j({ request, params }) {
  return await getOrderSettingsEndpoint.get(request, params);
}
async function action$i({ request, params }) {
  return await updateOrderSettingsEndpoint.post(request, params);
}
...
const loader$i = async (o) => await tangoVendedorGetAllEndpoint.get(o.request, o.params);
const action$h = unimplementedApiResponse;
...
async function userEntityToPublicInfo(user) {
  const { user_warning_message_title: title, user_warning_message_content: content } = await settingsProvider.getMiscSettings();
...
const loader$h = unimplementedApiResponse;
...
async function getPedidosByIds(user, ids) {
  ...
  const ordersById = new Map(orders.map((p) => [p.id, p]));
  const rows = await getAllPedidosRows(user, false);
  return ids.map((id) => {
    const order = ordersById.get(id);
    if (order == null) return null;
    const orderRows = rows[id];
    if (orderRows == null) return null;
    const newTotal = orderRows.reduce((total, row) => {
      const precioBonificado = applyTangoDiscount(row.precio, order.descuento) ?? 0;
      return total + precioBonificado * row.cantidad;
    }, 0);
    return {
      cabecera: {
        ...order,
...
const getMiscSettingsEndpoint = createApiEndpoint(
  ...
  async (req) => {
    const {
      admin_password_hash,
      ...remaining
    } = await settingsProvider.getMiscSettings(true);
    return remaining;
  }
);
const updateMiscSettingsEndpoint = createApiEndpoint(
  ...
  async (req) => {
    const { body } = req.validated;
    const oldSettings = await settingsProvider.getMiscSettings();
    const newSettingsDTO = new StoredMiscSettingsDTO({
      admin_password_hash: oldSettings.admin_password_hash,
      ...body
    });
    await settingsProvider.setMiscSettings(newSettingsDTO);
    return body;
  }
  /* *************************************************************************************************************** */
);
...
const loader$e = async (o) => await tangoClienteGetAllEndpoint.get(o.request, o.params);
const action$d = unimplementedApiResponse;
...
const getAdminStatusEndpoint = createApiEndpoint(
  ...
  /* *************************************************************************************************************** */
  async (req) => {
    const dbSettings = await settingsProvider.getDBSettings(true);
    const miscSettings = await settingsProvider.getMiscSettings(true);
...
const loader$c = unimplementedApiResponse;
...
const loader$a = async (o) => await tangoPerfilGetAllEndpoint.get(o.request, o.params);
...
const loader$9 = unimplementedApiResponse;
...
const getDBSettingsEndpoint = createApiEndpoint(
  ...
  void 0,
  async (req) => {
    const storedDBSettings = await settingsProvider.getDBSettings(true);
    const {
      tango_active_company,
      ...remaining
    } = storedDBSettings;
...
const updateDBSettingsEndpoint = createApiEndpoint(
  ...
  async (req) => {
    const { body } = req.validated;
    const oldSettings = await settingsProvider.getDBSettings();
    const newSettings = {
      ...body,
    ...
    await settingsProvider.setDBSettings(finalSettingsDTO);
...
const loader$6 = async (o) => await tangoListaDePreciosAllEndpoint.get(o.request, o.params);
...
const loader$5 = unimplementedApiResponse;
...
async function loader() {
  ...
  return {
    programConfig,
    programName: `${programNameFromNode} ${programVersionFromNode}`
  };
}
function Index() {
  const { programConfig, programName } = useLoaderData();
  useTheme();
  const { state: authState, dispatch: authDispatch } = useAuth();
  const dialogCall = useMessageToUserDialog();
  const { welcome_message } = programConfig;
  const {
    handleSubmit,
    control,
    reset,
        ...
        children: [
          /* @__PURE__ */ jsx(
            Container,
            {
              ...
              children: /* @__PURE__ */ jsxs(Card, { overflow: "hidden", children: [
                /* @__PURE__ */ jsx(CardHeader, { bgColor: "#ffffff", children: /* @__PURE__ */ jsx(Center, { children: /* @__PURE__ */ jsx(LogoImage, { width: "180px", alt: "" }) }) }),
          ...
          /* @__PURE__ */ jsx(Box, { marginTop: 5, textColor: "#AAA", textAlign: "center", children: programName })
        ]
      }
    )
  ] });
...
const serverManifest = { "entry": { "module": "/assets/entry.client-Cm5PaaAY.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/styleContexts-2JMs7XDn.js", "/assets/components-BuCL6aou.js"], "css": [] }, "routes": { "root": { "id": "root", "parentId": void 0, "path": "", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": true, "module": "/assets/root-CqKVlKJW.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/styleContexts-2JMs7XDn.js", "/assets/components-BuCL6aou.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/index-CIB9SwPf.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-AMBGAKG2-DYR7jJPr.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js"], "css": ["/assets/root-CrxCMxI6.css"] }, "routes/api.dxt.cliente.$id_cliente.siblings.$tango_id": { "id": "routes/api.dxt.cliente.$id_cliente.siblings.$tango_id", "parentId": "routes/api.dxt.cliente.$id_cliente", "path": "siblings/:tango_id", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.cliente._id_cliente.siblings._tango_id-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.users.customers.$id.edit": { "id": "routes/_admin.settings.users.customers.$id.edit", "parentId": "routes/_admin", "path": "settings/users/customers/:id/edit", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-CsnrtvER.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/index-DdrsuNi1.js", "/assets/index-BtnUei1e.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/useTangoList-Dsh644eR.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-VTV6N5LE-DMsBM71Z.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/vo_dxt_password-DQJmTFQe.js", "/assets/InlineError-D86JcCJL.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/utils-DQu-C9ov.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/settings-DlwZkTBe.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/schema_transformers-CZv4H_c_.js", "/assets/utils-DTXYOP_7.js"], "css": [] }, "routes/_admin.settings.users.sellers.$id.edit": { "id": "routes/_admin.settings.users.sellers.$id.edit", "parentId": "routes/_admin", "path": "settings/users/sellers/:id/edit", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-ClHKYuu5.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/index-DdrsuNi1.js", "/assets/index-BtnUei1e.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/useTangoList-Dsh644eR.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-VTV6N5LE-DMsBM71Z.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/vo_dxt_password-DQJmTFQe.js", "/assets/InlineError-D86JcCJL.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/utils-DQu-C9ov.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/settings-DlwZkTBe.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/schema_transformers-CZv4H_c_.js", "/assets/utils-DTXYOP_7.js"], "css": [] }, "routes/api.pedido.$id_pedido.start_new_draft": { "id": "routes/api.pedido.$id_pedido.start_new_draft", "parentId": "routes/api.pedido.$id_pedido", "path": "start_new_draft", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido._id_pedido.start_new_draft-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft.$id_pedido.start_new_order": { "id": "routes/api.draft.$id_pedido.start_new_order", "parentId": "routes/api.draft.$id_pedido", "path": "start_new_order", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft._id_pedido.start_new_order-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.users.customers.add": { "id": "routes/_admin.settings.users.customers.add", "parentId": "routes/_admin", "path": "settings/users/customers/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BAxkuK5X.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/index-BtnUei1e.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-VTV6N5LE-DMsBM71Z.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/vo_dxt_password-DQJmTFQe.js", "/assets/InlineError-D86JcCJL.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/utils-DQu-C9ov.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/settings-DlwZkTBe.js", "/assets/schema_transformers-CZv4H_c_.js", "/assets/SettingsFormHeading-389CUOcW.js"], "css": [] }, "routes/_authorized.drafts.$id_pedido.order": { "id": "routes/_authorized.drafts.$id_pedido.order", "parentId": "routes/_authorized", "path": "drafts/:id_pedido/order", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-CZR3xD7p.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.orders.$id_pedido.draft": { "id": "routes/_authorized.orders.$id_pedido.draft", "parentId": "routes/_authorized", "path": "orders/:id_pedido/draft", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-IbsVmGAj.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_admin.settings.product_list.print": { "id": "routes/_admin.settings.product_list.print", "parentId": "routes/_admin", "path": "settings/product_list/print", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-TJ9TVt7K.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/index-DdrsuNi1.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/index-CIB9SwPf.js", "/assets/utils-DQu-C9ov.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/index-Lfe6_JQR.js"], "css": [] }, "routes/_admin.settings.users.$type._index": { "id": "routes/_admin.settings.users.$type._index", "parentId": "routes/_admin", "path": "settings/users/:type", "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-jlvAC2tC.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/settings-DlwZkTBe.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ConfirmationDialog-B2vV8YvE.js", "/assets/SearchField-gbhuOeMX.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-MGVPL3OH-DdtIvv8f.js", "/assets/chunk-6RSEZNRH-p2hFl890.js"], "css": [] }, "routes/_authorized.drafts.$id_pedido.copy": { "id": "routes/_authorized.drafts.$id_pedido.copy", "parentId": "routes/_authorized", "path": "drafts/:id_pedido/copy", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-CAxSdsma.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.drafts.$id_pedido.edit": { "id": "routes/_authorized.drafts.$id_pedido.edit", "parentId": "routes/_authorized", "path": "drafts/:id_pedido/edit", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BimejLzj.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.orders.$id_pedido.copy": { "id": "routes/_authorized.orders.$id_pedido.copy", "parentId": "routes/_authorized", "path": "orders/:id_pedido/copy", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-Ci3S4SGz.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.orders.$id_pedido.edit": { "id": "routes/_authorized.orders.$id_pedido.edit", "parentId": "routes/_authorized", "path": "orders/:id_pedido/edit", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-D_5JS5Vv.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/api.dxt.cliente.siblings.$tango_id": { "id": "routes/api.dxt.cliente.siblings.$tango_id", "parentId": "routes/api.dxt.cliente", "path": "siblings/:tango_id", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.cliente.siblings._tango_id-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.$id_pedido.start_update": { "id": "routes/api.pedido.$id_pedido.start_update", "parentId": "routes/api.pedido.$id_pedido", "path": "start_update", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido._id_pedido.start_update-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.product_list.edit": { "id": "routes/_admin.settings.product_list.edit", "parentId": "routes/_admin", "path": "settings/product_list/edit", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-cre7JpbE.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/index-DdrsuNi1.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/index-CIB9SwPf.js", "/assets/utils-DQu-C9ov.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/index-Lfe6_JQR.js"], "css": [] }, "routes/_admin.settings.users.sellers.add": { "id": "routes/_admin.settings.users.sellers.add", "parentId": "routes/_admin", "path": "settings/users/sellers/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-DZ4iD9Fp.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/index-BtnUei1e.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-RDF2AYID-Ckn2v1iE.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/chunk-VTV6N5LE-DMsBM71Z.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/vo_dxt_password-DQJmTFQe.js", "/assets/InlineError-D86JcCJL.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/utils-DQu-C9ov.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/settings-DlwZkTBe.js", "/assets/schema_transformers-CZv4H_c_.js", "/assets/SettingsFormHeading-389CUOcW.js"], "css": [] }, "routes/api.draft.$id_pedido.start_update": { "id": "routes/api.draft.$id_pedido.start_update", "parentId": "routes/api.draft.$id_pedido", "path": "start_update", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft._id_pedido.start_update-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.$id_pedido.start_copy": { "id": "routes/api.pedido.$id_pedido.start_copy", "parentId": "routes/api.pedido.$id_pedido", "path": "start_copy", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido._id_pedido.start_copy-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.start_new.$id_cliente": { "id": "routes/api.pedido.start_new.$id_cliente", "parentId": "routes/api.pedido.start_new", "path": ":id_cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido.start_new._id_cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft.$id_pedido.start_copy": { "id": "routes/api.draft.$id_pedido.start_copy", "parentId": "routes/api.draft.$id_pedido", "path": "start_copy", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft._id_pedido.start_copy-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft.start_new.$id_cliente": { "id": "routes/api.draft.start_new.$id_cliente", "parentId": "routes/api.draft.start_new", "path": ":id_cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft.start_new._id_cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.articulo.print_list.ids": { "id": "routes/api.dxt.articulo.print_list.ids", "parentId": "routes/api.dxt.articulo.print_list", "path": "ids", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.articulo.print_list.ids-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_authorized.drafts.$client.add": { "id": "routes/_authorized.drafts.$client.add", "parentId": "routes/_authorized", "path": "drafts/:client/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-Bs2Hxn-B.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.orders.$client.add": { "id": "routes/_authorized.orders.$client.add", "parentId": "routes/_authorized", "path": "orders/:client/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-D0dn9n0u.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/api.dictionary.active_company": { "id": "routes/api.dictionary.active_company", "parentId": "routes/api.dictionary", "path": "active_company", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dictionary.active_company-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft.$id_pedido.favorite": { "id": "routes/api.draft.$id_pedido.favorite", "parentId": "routes/api.draft.$id_pedido", "path": "favorite", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft._id_pedido.favorite-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.vendedor.$id_vendedor": { "id": "routes/api.dxt.vendedor.$id_vendedor", "parentId": "routes/api.dxt.vendedor", "path": ":id_vendedor", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.vendedor._id_vendedor-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_authorized.change_password": { "id": "routes/_authorized.change_password", "parentId": "routes/_authorized", "path": "change_password", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BezdM96s.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/Navbar-DANenbq-.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/app_resources-Di8KxAHS.js", "/assets/refresh_all-CbFR3A5j.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/vo_dxt_password-DQJmTFQe.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/utils-DQu-C9ov.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js"], "css": [] }, "routes/api.dxt.articulo.print_list": { "id": "routes/api.dxt.articulo.print_list", "parentId": "root", "path": "api/dxt/articulo/print_list", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.articulo.print_list-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.cliente.$id_cliente": { "id": "routes/api.dxt.cliente.$id_cliente", "parentId": "routes/api.dxt.cliente", "path": ":id_cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.cliente._id_cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.articulo.edit_list": { "id": "routes/api.dxt.articulo.edit_list", "parentId": "root", "path": "api/dxt/articulo/edit_list", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.articulo.edit_list-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.usuario.auxiliares": { "id": "routes/api.dxt.usuario.auxiliares", "parentId": "root", "path": "api/dxt/usuario/auxiliares", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.usuario.auxiliares-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_authorized.drafts._index": { "id": "routes/_authorized.drafts._index", "parentId": "routes/_authorized", "path": "drafts", "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BHY1MYQw.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/TextPrice-pYuEWncP.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/Navbar-DANenbq-.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/SearchField-gbhuOeMX.js", "/assets/index-BdVeZ32W.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/ConfirmationDialog-B2vV8YvE.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-K7XRJ7NL-BmZBywqz.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-CWVAJCXJ-BZwDvuOt.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/index-UZYW6Eeq.js"], "css": [] }, "routes/_authorized.orders._index": { "id": "routes/_authorized.orders._index", "parentId": "routes/_authorized", "path": "orders", "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BvZr9QRk.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/TextPrice-pYuEWncP.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/Navbar-DANenbq-.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/SearchField-gbhuOeMX.js", "/assets/index-BdVeZ32W.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/AccountCancelIcon-B2VjX7I2.js", "/assets/ConfirmationDialog-B2vV8YvE.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-K7XRJ7NL-BmZBywqz.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-CWVAJCXJ-BZwDvuOt.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/index-UZYW6Eeq.js"], "css": [] }, "routes/_authorized.orders.print": { "id": "routes/_authorized.orders.print", "parentId": "routes/_authorized", "path": "orders/print", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-KRfUajfR.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index-BtnUei1e.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/utils-DTXYOP_7.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-BdVeZ32W.js", "/assets/chunk-MGVPL3OH-DdtIvv8f.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-VTV6N5LE-DMsBM71Z.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js"], "css": [] }, "routes/api.dxt.vendedor.cliente": { "id": "routes/api.dxt.vendedor.cliente", "parentId": "routes/api.dxt.vendedor", "path": "cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.vendedor.cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.company": { "id": "routes/_admin.settings.company", "parentId": "routes/_admin", "path": "settings/company", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BApfmmmN.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/index-BtnUei1e.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app-DHK1Xtws.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/app_resources-Di8KxAHS.js", "/assets/useTangoList-Dsh644eR.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/InlineError-D86JcCJL.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js"], "css": [] }, "routes/_admin.settings._index": { "id": "routes/_admin.settings._index", "parentId": "routes/_admin", "path": "settings", "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-DNb3cRxP.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js"], "css": [] }, "routes/_authorized.drafts.add": { "id": "routes/_authorized.drafts.add", "parentId": "routes/_authorized", "path": "drafts/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-ctdKiJer.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/_authorized.orders.add": { "id": "routes/_authorized.orders.add", "parentId": "routes/_authorized", "path": "orders/add", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BVICn3wh.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/TextPrice-pYuEWncP.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/index.esm-CIB2f2-Q.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/order.api_hooks-DBtEYjHv.js", "/assets/OrdersContainer-lqr2uFIj.js", "/assets/SearchField-gbhuOeMX.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/Navbar-DANenbq-.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/chunk-T2WCTPDH-B3SegIkI.js", "/assets/useTangoList-Dsh644eR.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledSelect-DDMsMwex.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/index-CoXMS8wA.js", "/assets/chunk-Z6RXEUPO-CxfasoKf.js", "/assets/chunk-6RSEZNRH-p2hFl890.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/CommonErrors-Dv5kaEHh.js", "/assets/index-kN_ca5B9.js"], "css": [] }, "routes/api.download.$filename": { "id": "routes/api.download.$filename", "parentId": "routes/api.download", "path": ":filename", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.download._filename-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.order": { "id": "routes/_admin.settings.order", "parentId": "routes/_admin", "path": "settings/order", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-gBH0vhM8.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index-BtnUei1e.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-CWVAJCXJ-BZwDvuOt.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/requests-CGosjl0w.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ControlledCheckbox-tWi5bC4c.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/chunk-6RSEZNRH-p2hFl890.js"], "css": [] }, "routes/_admin.settings.tango": { "id": "routes/_admin.settings.tango", "parentId": "routes/_admin", "path": "settings/tango", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-Db4YJ9PF.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/requests-CGosjl0w.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/utils-DQu-C9ov.js", "/assets/misc_settings.consts-CNGs7G2v.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-6RSEZNRH-p2hFl890.js"], "css": [] }, "routes/_authorized.downloads": { "id": "routes/_authorized.downloads", "parentId": "routes/_authorized", "path": "downloads", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-C6Jf4_UN.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/components-BuCL6aou.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/Navbar-DANenbq-.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/chunk-AMBGAKG2-DYR7jJPr.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/FormSkeletons-Ch1cmlVo.js"], "css": [] }, "routes/api.pedido.$id_pedido": { "id": "routes/api.pedido.$id_pedido", "parentId": "routes/api.pedido", "path": ":id_pedido", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido._id_pedido-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin.settings.misc": { "id": "routes/_admin.settings.misc", "parentId": "routes/_admin", "path": "settings/misc", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BBwCXLT7.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/chunk-VMD3UMGK-D9IL_j_8.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/ResponsiveIconButton-CQwgfxML.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/CloseIcon-B6Be6esX.js", "/assets/app_resources-Di8KxAHS.js", "/assets/app-DHK1Xtws.js", "/assets/components-BuCL6aou.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/ApiErrors-Blpol96Y.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/index-BtnUei1e.js", "/assets/index-DdrsuNi1.js", "/assets/chunk-7D6N5TE5-BjI2zV3K.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-CWVAJCXJ-BZwDvuOt.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/index-CIB9SwPf.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-3Y4YXCR2-WVOthZPF.js", "/assets/SettingsFormHeading-389CUOcW.js", "/assets/requests-CGosjl0w.js", "/assets/useDXTApiFetch-D7fvlkDE.js", "/assets/FormSkeletons-Ch1cmlVo.js", "/assets/chunk-ZPFGWTBB-Bu2gzu07.js", "/assets/ControlledCheckbox-tWi5bC4c.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/ControlledTextarea-CtVdSOvZ.js", "/assets/FormErrors-COO1VgQ3.js", "/assets/SettingsFormButtons-BcygzEqX.js", "/assets/utils-DQu-C9ov.js", "/assets/misc_settings.consts-CNGs7G2v.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-6RSEZNRH-p2hFl890.js"], "css": [] }, "routes/api.draft.$id_pedido": { "id": "routes/api.draft.$id_pedido", "parentId": "routes/api.draft", "path": ":id_pedido", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft._id_pedido-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.renglones": { "id": "routes/api.pedido.renglones", "parentId": "routes/api.pedido", "path": "renglones", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido.renglones-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.start_new": { "id": "routes/api.pedido.start_new", "parentId": "routes/api.pedido", "path": "start_new", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido.start_new-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_authorized.version": { "id": "routes/_authorized.version", "parentId": "routes/_authorized", "path": "version", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-Bv0JyOgO.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/Navbar-DANenbq-.js", "/assets/CommonCard-CfkzH5Am.js", "/assets/chunk-K7XRJ7NL-BmZBywqz.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-W7WUSNWJ-CwvPeB9a.js", "/assets/chunk-QINAG4RG-DZB8sNy9.js", "/assets/components-BuCL6aou.js"], "css": [] }, "routes/api.draft.renglones": { "id": "routes/api.draft.renglones", "parentId": "routes/api.draft", "path": "renglones", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft.renglones-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft.start_new": { "id": "routes/api.draft.start_new", "parentId": "routes/api.draft", "path": "start_new", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft.start_new-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.settings.order": { "id": "routes/api.settings.order", "parentId": "root", "path": "api/settings/order", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.settings.order-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.tango.vendedor": { "id": "routes/api.tango.vendedor", "parentId": "root", "path": "api/tango/vendedor", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.tango.vendedor-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.auth.password": { "id": "routes/api.auth.password", "parentId": "root", "path": "api/auth/password", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.auth.password-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.pedido.by_ids": { "id": "routes/api.pedido.by_ids", "parentId": "routes/api.pedido", "path": "by_ids", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido.by_ids-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.settings.misc": { "id": "routes/api.settings.misc", "parentId": "root", "path": "api/settings/misc", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.settings.misc-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.tango.cliente": { "id": "routes/api.tango.cliente", "parentId": "root", "path": "api/tango/cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.tango.cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.admin.status": { "id": "routes/api.admin.status", "parentId": "root", "path": "api/admin/status", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.admin.status-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.auth.connect": { "id": "routes/api.auth.connect", "parentId": "root", "path": "api/auth/connect", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.auth.connect-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.vendedor": { "id": "routes/api.dxt.vendedor", "parentId": "root", "path": "api/dxt/vendedor", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.vendedor-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.tango.perfil": { "id": "routes/api.tango.perfil", "parentId": "root", "path": "api/tango/perfil", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.tango.perfil-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.auth.logout": { "id": "routes/api.auth.logout", "parentId": "root", "path": "api/auth/logout", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.auth.logout-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dxt.cliente": { "id": "routes/api.dxt.cliente", "parentId": "root", "path": "api/dxt/cliente", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dxt.cliente-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.settings.db": { "id": "routes/api.settings.db", "parentId": "root", "path": "api/settings/db", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.settings.db-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.tango.lista": { "id": "routes/api.tango.lista", "parentId": "root", "path": "api/tango/lista", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.tango.lista-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.auth.login": { "id": "routes/api.auth.login", "parentId": "root", "path": "api/auth/login", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.auth.login-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.dictionary": { "id": "routes/api.dictionary", "parentId": "root", "path": "api/dictionary", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.dictionary-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.download": { "id": "routes/api.download", "parentId": "root", "path": "api/download", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.download-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_authorized": { "id": "routes/_authorized", "parentId": "root", "path": void 0, "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-BbYSsDwj.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/refresh_all-CbFR3A5j.js", "/assets/AuthGuard-DZ8YUfo4.js", "/assets/ClientOnly-CruWkRVx.js"], "css": [] }, "routes/api.pedido": { "id": "routes/api.pedido", "parentId": "root", "path": "api/pedido", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.pedido-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/api.draft": { "id": "routes/api.draft", "parentId": "root", "path": "api/draft", "index": void 0, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/api.draft-l0sNRNKZ.js", "imports": [], "css": [] }, "routes/_admin": { "id": "routes/_admin", "parentId": "root", "path": void 0, "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-CxzdJI-x.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/refresh_all-CbFR3A5j.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/index-CIB9SwPf.js", "/assets/index-BtnUei1e.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-QURMB2UJ-Dpy63TOY.js", "/assets/chunk-KRPLQIP4-Duu5zv9H.js", "/assets/program_config.consts-tkpR5RSB.js", "/assets/order_settings.consts-BS2ZOo_e.js", "/assets/chunk-HB6KBUMZ-BghbaEwP.js", "/assets/chunk-3ASUQ6PA-BkEgKCWb.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-BL2ZZSHG-BDLdsFeM.js", "/assets/AuthGuard-DZ8YUfo4.js", "/assets/ClientOnly-CruWkRVx.js", "/assets/Navbar-DANenbq-.js"], "css": [] }, "routes/_index": { "id": "routes/_index", "parentId": "root", "path": void 0, "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasErrorBoundary": false, "module": "/assets/route-jwGB4uQX.js", "imports": ["/assets/emotion-element-5486c51c.browser.esm-CMT9BBIk.js", "/assets/auth_context-CCfWU2Jo.js", "/assets/app-DHK1Xtws.js", "/assets/chunk-6QYXN73V-tPml5wjb.js", "/assets/chunk-56K2BSAJ-Bys-b2nt.js", "/assets/chunk-PULVB27S-C1tz_n3L.js", "/assets/index.esm-CzxlHhdA.js", "/assets/chunk-H46NUPBZ-CGUTFX_3.js", "/assets/chunk-6CVSDS6C-BMnlnkqH.js", "/assets/ColorModeSelector-zA6QYbuT.js", "/assets/chunk-7OLJDQMT-DqhdIuo6.js", "/assets/chunk-2OOHT3W5-CC38uFWT.js", "/assets/chunk-MFVQSVQB-Bvk0kezP.js", "/assets/utils-DQu-C9ov.js", "/assets/vo_user_name-DRov8-rC.js", "/assets/ControlledInput-BKce-7V6.js", "/assets/chunk-QINAG4RG-DZB8sNy9.js", "/assets/MessageToUserDialog-CPklsPmb.js", "/assets/ClientOnly-CruWkRVx.js", "/assets/components-BuCL6aou.js", "/assets/chunk-YQO7BFFX-BA4DVzl3.js", "/assets/chunk-46CXQZ4E-CqQSj_hs.js", "/assets/chunk-NTCQBYKE-BqJT1CcA.js", "/assets/chunk-3KCBMPN5-D8G8ZRLu.js", "/assets/chunk-2ZHRCML3-Cd9Yb1VE.js"], "css": [] } }, "url": "/assets/manifest-d94fcd4d.js", "version": "d94fcd4d" };
...
const routes = {
  ...
  "routes/_admin.settings.product_list.print": {
    id: "routes/_admin.settings.product_list.print",
    parentId: "routes/_admin",
    path: "settings/product_list/print",
    index: void 0,
    caseSensitive: void 0,
    module: route9
  },
  ...
  "routes/_admin.settings.product_list.edit": {
    id: "routes/_admin.settings.product_list.edit",
    parentId: "routes/_admin",
    path: "settings/product_list/edit",
    index: void 0,
    caseSensitive: void 0,
    module: route17
  },
  ...
  "routes/_admin.settings.order": {
    id: "routes/_admin.settings.order",
    parentId: "routes/_admin",
    path: "settings/order",
    index: void 0,
    caseSensitive: void 0,
  ...
  "routes/_authorized.version": {
    id: "routes/_authorized.version",
    parentId: "routes/_authorized",
    path: "version",
    index: void 0,
    caseSensitive: void 0,
  ...
  "routes/api.settings.order": {
    id: "routes/api.settings.order",
    parentId: "root",
    path: "api/settings/order",
    index: void 0,
    caseSensitive: void 0,
    module: route55
  },
